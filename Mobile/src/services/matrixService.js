...
export async function getRoomName(accessToken, roomId, myUserId) {
  try {
    ...
    if (membersRes.ok) {
      ...
      const relevantMembers = chunk.filter(
        (e) =>
...
export async function syncMatrix(
  ...
  const res = await fetch(`${SYNAPSE_BASE}/_matrix/client/r0/sync?${params}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) {
...
export async function sendReaction(accessToken, roomId, eventId, key) {
  ...
  const res = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/send/m.reaction/${txnId}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export async function pinMessage(accessToken, roomId, eventId) {
  ...
  const stRes = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/state/m.room.pinned_events`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    },
  );
...
export async function forwardMessage(accessToken, targetRoomId, originalMsg) {
  ...
  return await fetch(
    `${SYNAPSE_BASE}/_matrix/client/r0/rooms/${encodeURIComponent(targetRoomId)}/send/m.room.message/${txnId}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export async function getRoomState(accessToken, roomId, eventType) {
  const res = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/state/${eventType}`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    },
  );
...
export async function getRoomMembers(accessToken, roomId) {
  ...
  return (data.chunk || []).map((e) => ({
    userId: e.state_key,
    displayName:
      e.content?.displayname || e.state_key.split(':')[0].replace('@', ''),
...
export async function leaveRoom(accessToken, roomId) {
  const res = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/leave`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
