...
function MembersPanel({
  ...
  const handleMemberPress = async (member) => {
    ...
    try {
      const dmRoomId = await getOrCreateDMRoom(
        accessToken,
        myUserId,
        member.userId,
      );
    ...
    >
      ...
      >
        ...
        <Animated.View
          style={[
            MP.innerContainer,
            { width: PANEL_WIDTH * 2, transform: [{ translateX: slideX }] },
          ]}
        >
          {/* ════════════════════════════════════════
              VIEW 1: Members List
          ...
          <View
            style={[
              MP.viewSlot,
              { width: PANEL_WIDTH, backgroundColor: theme.surface },
            ]}
          >
            ...
            <Text style={[MP.subheading, { color: textSecondary }]}>
              Tap a member to start a private chat
            </Text>
            ...
            {loading ? (
              <View style={MP.loaderWrap}>
                <ActivityIndicator
                  ...
                  size='large'
                />
                <Text style={[MP.loaderTxt, { color: textSecondary }]}>
                  Loading members…
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item: member }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <Text
                          style={[MP.memberInitial, { color: colors.text }]}
                        >
                        ...
                        <View
                          style={[
                            MP.presenceDot,
                            { backgroundColor: isMe ? '#22C55E' : '#94A3B8' },
                          ]}
                        />
                      </View>
                      <View style={MP.memberInfo}>
                        ...
                        <Text
                          style={[MP.memberUserId, { color: textSecondary }]}
                          numberOfLines={1}
                        >
                          {member.userId}
                        </Text>
                ...
                ListEmptyComponent={
                  <View style={MP.empty}>
                    <Text style={[MP.emptyTxt, { color: textSecondary }]}>
                      No members found
                    </Text>
                  </View>
                }
          ...
          >
            <View style={[MP.header, { backgroundColor: headerBg }]}>
              ...
              <Text style={[MP.headerTitle, { flex: 1, marginLeft: 6 }]}>
                Room Info
              </Text>
              {/* X: closes panel */}
              <TouchableOpacity
            ...
            <View style={{ flex: 1, padding: 16 }}>
                ...
                >
                  ...
                  {!!ticketId && (
                    <View style={{ marginBottom: roomName ? 10 : 0 }}>
                      <Text
                        style={[MP.infoCardLabel, { color: textSecondary }]}
                      >
                        Ticket ID
                      </Text>
                      <Text
              ...
              <Text style={[MP.sectionLabel, { color: textSecondary }]}>
                Actions
              </Text>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[MP.actionTitle, { color: '#DC2626' }]}>
                    {leavingRoom ? 'Leaving…' : 'Leave Room'}
                  </Text>
                  <Text style={[MP.actionSub, { color: textSecondary }]}>
                    Remove yourself from this chat
                  </Text>
                </View>
                <Ionicons
                  name='chevron-forward'
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[MP.actionTitle, { color: textPrimary }]}>
                    Logout
                  </Text>
                  <Text style={[MP.actionSub, { color: textSecondary }]}>
...
export default function ChatRoomListScreen({ extraRoomId, autoOpenRoomId }) {
  ...
  const resolveDisplayName = (rid) => {
    const ticketInfo = ticketNames[rid];
    if (ticketInfo?.name) return ticketInfo.name;
    return roomDisplayName(rid, roomNames, dmNames);
  };
  ...
  useEffect(() => {
    AsyncStorage.getItem('TICKET_NAMES').then((raw) => {
      if (raw) {
        try {
          setTicketNames(JSON.parse(raw));
        } catch (e) {}
      }
    });
    AsyncStorage.getItem('ROOM_NAMES').then((raw) => {
      if (raw) {
        try {
          setRoomNames((prev) => ({ ...JSON.parse(raw), ...prev }));
        } catch (e) {}
      }
    });
    AsyncStorage.getItem('ROOM_LIST_CACHE').then((raw) => {
      if (raw) {
        try {
  ...
  useEffect(() => {
    if (autoOpenRoomId) {
      ...
      setOpenLabel(resolveDisplayName(autoOpenRoomId));
    }
  }, [autoOpenRoomId]);
  ...
  useEffect(() => {
    if (openRoomId) setOpenLabel(resolveDisplayName(openRoomId));
  }, [roomNames, dmNames, ticketNames]);

  const loadRooms = async () => {
    if (!session?.accessToken) return;
    try {
      setLoading(rooms.length === 0);
      // Refresh patient/ticket names in background — don't block the room list.
      fetchIncidents();

      const joined = await getJoinedRooms(session.accessToken);
      const allIds = Array.from(
        new Set([...(extraRoomId ? [extraRoomId] : []), ...joined]),
      );

      // Render rooms immediately with whatever cached data we have, so the user
      // sees the list instantly while names + previews resolve in the background.
      setRooms((prev) => {
        const map = {};
        prev.forEach((r) => {
          map[r.roomId] = r;
        ...
        return allIds.map(
          (rid) => map[rid] || { roomId: rid, lastEvent: null, unread: 0 },
        );
      });
      setLoading(false);
      ...
      // Resolve room names in parallel (was sequential — biggest bottleneck).
      const nameResults = await Promise.all(
        allIds.map(async (rid) => {
          if (roomNames[rid]) return [rid, roomNames[rid]];
          try {
            const n = await getRoomName(
              session.accessToken,
              rid,
              session.userId,
            );
      ...
      nameResults.forEach(([rid, n]) => {
        if (n && nextNames[rid] !== n) {
          nextNames[rid] = n;
          namesChanged = true;
        }
      });
      if (namesChanged) {
        setRoomNames(nextNames);
        AsyncStorage.setItem('ROOM_NAMES', JSON.stringify(nextNames)).catch(
          () => {},
        );
      }

      // Only need the most recent message for the preview — was fetching 20.
      const roomData = await Promise.all(
        allIds.map(async (roomId) => {
          try {
            ...
            const events = (data.chunk || []).filter(
      ...
      roomData.sort(
        (a, b) =>
          (b.lastEvent?.origin_server_ts || 0) -
          (a.lastEvent?.origin_server_ts || 0),
      );
      setRooms(roomData);
      AsyncStorage.setItem('ROOM_LIST_CACHE', JSON.stringify(roomData)).catch(
        () => {},
      );
    } catch (err) {
      ...
      while (syncActive.current) {
        try {
          ...
          for (const invitedRoomId in invitedRooms) {
            try {
              const { joinRoom, saveDirectMessage } =
                await import('../services/matrixService');
              await joinRoom(session.accessToken, invitedRoomId);
              newRoomsFound = true;
              const inviteState =
                invitedRooms[invitedRoomId].invite_state?.events || [];
              const isDirect =
                inviteState.find((e) => e.type === 'm.room.create')?.content
                  ?.is_direct === true;
              if (isDirect) {
                const me = inviteState.find(
                  (e) =>
          ...
          if (newRoomsFound || Object.keys(updatedRooms).length > 0) {
            ...
            setRooms((prev) => {
              ...
              Object.entries(updatedRooms).forEach(([rId, rData]) => {
                if (!map[rId])
                  map[rId] = { roomId: rId, lastEvent: null, unread: 0 };
                const events = (rData.timeline?.events || []).filter(
                  (e) => e.type === 'm.room.message',
                );
                if (events.length > 0) {
                  ...
                  const incomingUnread = events.filter(
                    (e) =>
                      e.sender !== session.userId &&
                      rId !== openRoomIdRef.current,
                  ).length;
  ...
  const openRoom = (room, label) => {
    ...
    setOpenLabel(label || resolveDisplayName(room.roomId));
  ...
  const openRoomIsTicket = openRoomRawName
    ?.trim()
    ?.toLowerCase()
    ?.includes('ticket-');
    ...
    return (
      <View style={[styles.flex, { backgroundColor: theme.bg }]}>
        <View
          style={[
            styles.chatHeader,
            { paddingTop: insets.top + 10, backgroundColor: theme.topBar },
          ]}
        >
          <TouchableOpacity
            onPress={closeRoom}
          ...
          <View
            style={[styles.chatHeaderAvatar, { backgroundColor: colors.bg }]}
          >
            <Text style={[styles.chatHeaderAvatarTxt, { color: colors.text }]}>
              {getInitials(openLabel || resolveDisplayName(openRoomId))}
            </Text>
          </View>
          <View style={styles.chatHeaderInfo}>
            ...
            <Text style={styles.chatHeaderSub}>
              {isDM ? '🔒 Private conversation' : 'Alert Communication'}
            </Text>
          </View>
          <TouchableOpacity
            style={[
              styles.membersToggleBtn,
              membersVisible && styles.membersToggleBtnActive,
            ]}
  ...
  const filtered = rooms.filter((r) => {
    ...
    if (!q) return true;
    const base = roomDisplayName(r.roomId, roomNames, dmNames).toLowerCase();
    const patient = (ticketNames[r.roomId]?.name || '').toLowerCase();
    return base.includes(q) || patient.includes(q);
  });
  ...
  const renderRoom = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.roomRow, { backgroundColor: theme.surface }]}
        onPress={() => openRoom(item, displayName)}
        activeOpacity={0.75}
      >
        <View style={[styles.avatar, { backgroundColor: colors.bg }]}>
          <Text style={[styles.avatarTxt, { color: colors.text }]}>
            {getInitials(displayName)}
          </Text>
          {isDMRoom && (
            <View style={[styles.onlineDot, { backgroundColor: '#A78BFA' }]} />
          )}
        </View>
        <View style={styles.roomInfo}>
          <View style={styles.roomNameRow}>
            <View style={styles.roomNameWrap}>
              {isDMRoom && (
                <View
                  style={[styles.dmTag, { backgroundColor: theme.accentBg }]}
                >
                  <Text style={[styles.dmTagTxt, { color: theme.accent }]}>
                    DM
                  </Text>
                </View>
              )}
          ...
          {!!(isTicketRoom && cleanTicketId) && (
            <View style={{ marginBottom: 2 }}>
              <Text
                style={{ fontSize: 10, color: '#10B981', fontWeight: 'bold' }}
              >
                {cleanTicketId}
              </Text>
            </View>
            ...
            <View style={styles.previewLeft}>
              ...
              <Text
                style={[
                  styles.roomPreview,
                  { color: theme.textSecondary },
                  hasUnread && { color: theme.textPrimary, fontWeight: '500' },
                ]}
                numberOfLines={1}
              >
      ...
      {loading ? (
        <View style={styles.loader}>
          <ActivityIndicator
            ...
            color={theme.accent}
          />
          <Text style={[styles.loaderTxt, { color: theme.textSecondary }]}>
            Loading chats…
          </Text>
        </View>
      ) : filtered.length === 0 ? (
        <View style={styles.emptyWrap}>
          <Ionicons
            ...
            color={theme.accent}
          />
          <Text style={[styles.emptyTxt, { color: theme.textPrimary }]}>
            No chats yet
          </Text>
