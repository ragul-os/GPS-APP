    ...
    const t = Math.max(
      ...
      Math.min(
        ...
        ((pos.latitude - A.latitude) * (B.latitude - A.latitude) +
          (pos.longitude - A.longitude) * (B.longitude - A.longitude)) /
...
const smoothPos = (prev, next, alpha = 0.45) => {
  if (!prev) return next;
  return {
    latitude: prev.latitude * (1 - alpha) + next.latitude * alpha,
    longitude: prev.longitude * (1 - alpha) + next.longitude * alpha,
  };
};
...
const getManeuverIcon = (m = '', i = '') => {
  const mv = m.toLowerCase(),
    ins = i.toLowerCase();
  if (mv.includes('turn-left') || ins.includes('turn left'))
    return 'arrow-back';
  if (mv.includes('turn-right') || ins.includes('turn right'))
    return 'arrow-forward';
  if (mv.includes('uturn') || ins.includes('u-turn')) return 'return-up-back';
  if (mv.includes('slight-left') || ins.includes('slight left'))
    return 'arrow-back-outline';
  if (mv.includes('slight-right') || ins.includes('slight right'))
...
const trafficInfo = (durS, traffS) => {
  ...
  const r = traffS / durS;
  if (r < 1.1)
    return { label: 'Free Flow', color: '#1A73E8', light: '#E8F0FE' };
  if (r < 1.3) return { label: 'Moderate', color: '#F9A825', light: '#FFF8E1' };
  if (r < 1.6)
    return { label: 'Heavy Traffic', color: '#E53935', light: '#FFEBEE' };
...
const fmtArrival = (remSeconds) => {
  const d = new Date(Date.now() + remSeconds * 1000);
  let h = d.getHours(),
    m = d.getMinutes();
  const ap = h >= 12 ? 'PM' : 'AM';
  h = h % 12 || 12;
...
const postWithRetry = async (url, body, label) => {
  for (let attempt = 1; attempt <= POST_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      clearTimeout(timer);
      console.log(
        `✅ ${label} SUCCESS: ${r.status} (attempt ${attempt}/${POST_MAX_ATTEMPTS})`,
      );
      return r;
    } catch (e) {
      clearTimeout(timer);
      ...
      console.error(
        `❌ ${label} FAILED (attempt ${attempt}/${POST_MAX_ATTEMPTS}): ${isTimeout ? 'timeout after 10s' : e.message}`,
      );
      if (attempt >= POST_MAX_ATTEMPTS) return null;
...
const notifyStatus = (unitId, status, ticketNo) => {
  ...
  const teEvent = TRIP_STATUS_TO_EVENT[status];
  if (ticketNo && teEvent) {
    fetch(
      `${WEBHOOK_URL}/webhook/ticket-event/${encodeURIComponent(ticketNo)}/unit`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.warn(`[ticket-events] ${teEvent} failed:`, err.message),
    );
  }
...
export default function MapScreen({ route: navRoute, navigation }) {
  ...
  const ticketNo =
    navRoute?.params?.ticketNo ||
    navRoute?.params?.incidentId ||
    navRoute?.params?.ticket_no ||
    '';
  ...
  const [tripStatus, setTripStatus] = useState(
    navRoute?.params?.initialTripStatus || 'accepted',
  );
  ...
  const unitType =
    session?.unitType || navRoute?.params?.unitType || 'ambulance';
    ...
    if (next) {
      ...
      if (activeR && navRef.current === MODE.NAVIGATE) {
        ...
        if (currentStep) {
          ...
          Speech.speak(`${distLabel}${currentStep.instruction}`, {
            language: 'en-IN',
            rate: 0.92,
          });
        }
      ...
      (loc) => {
        ...
        if (navRef.current === MODE.NAVIGATE) {
          ...
          if (activeR) {
            ...
            let minD = Infinity,
              bestIdx = routeProgressIdxRef.current;
            const searchEnd = Math.min(
              poly.length,
              routeProgressIdxRef.current + 50,
            );
            for (let i = routeProgressIdxRef.current; i < searchEnd; i++) {
              const d = dist(pos, poly[i]);
              if (d < minD) {
                minD = d;
                bestIdx = i;
            ...
            if (bestIdx > routeProgressIdxRef.current)
              routeProgressIdxRef.current = bestIdx;
            ...
            const newRem = {
              distM: Math.round(remM),
              timeS: Math.round(activeR.trafficSeconds * fracLeft),
            };
            setRemaining(newRem);
            ...
            if (
              curStep?.endLocation &&
              dist(pos, curStep.endLocation) < 25 &&
              stepIdxRef.current < activeR.steps.length - 1
            ) {
            ...
            if (
              ...
              (currentSt === 'en_route' || currentSt === 'on_action')
            ) {
              const dToDest = dist(pos, destRef.current);
              if (dToDest < 50) {
                tripStatusRef.current = 'arrived';
                setTripStatus('arrived');
                notifyStatus(ambulanceId, 'arrived', ticketNo);
              }
            }
            if (dFromPrev > 3 && bearingSamplesRef.current >= 3) {
              ...
              if (routeBear !== null) {
                ...
                if (diff > 80) {
                  setWrongDir(true);
                  setWrongDirDeg(Math.round(diff));
                  if (!wrongDirStart.current) wrongDirStart.current = now;
                  else if (
                    now - wrongDirStart.current > 5000 &&
                    cooldownOk &&
                    !fetchingRef.current
                  ) {
                    wrongDirStart.current = null;
                    reRouteRef.current = now;
                    offRoadCount.current = 0;
                    routeProgressIdxRef.current = 0;
                    bearingSamplesRef.current = 0;
          ...
          const ch = movBearingRef.current;
          cameraHdgRef.current = ch;
          const dTurn = distToTurnRef.current;
          ...
          mapRef.current?.animateCamera(
            {
              ...
              zoom,
              heading: ch,
              pitch: isWrong ? 0 : dTurn < 40 ? 35 : 45,
            },
            { duration: isWrong ? 400 : dTurn < 40 ? 180 : 320 },
          );
          return;
        }
        if (!onceFetched.current && !fetchingRef.current) {
          doFetch(pos, destRef.current, tModeRef.current, false, true);
          return;
        }
        const activeR2 = routesRef.current[selRef.current];
        if (activeR2)
          setTrimmed(
            trimFromCar(activeR2.roadPoly, pos, routeProgressIdxRef.current),
          );
      },
    );
  };

  // REPLACE WITH — pings every 3 seconds with overlap protection
  const pingInFlight = useRef(false); // ADD THIS REF at top of component

  const startPing = () => {
    pingTimer.current = setInterval(async () => {
      if (!carRef.current) return;
      if (pingInFlight.current) return; // skip if previous ping still running
      ...
      try {
        await ping(
          ...
          movBearingRef.current,
          speedRef.current,
          remainingRef.current.distM,
          remainingRef.current.timeS,
          tripStatusRef.current,
          stepIdxRef.current,
          routesRef.current[selRef.current]?.steps?.length || 0,
          carRef.current ? dist(carRef.current, destRef.current) : 0,
          ticketNo,
        );
      } catch (err) {
    ...
    }, 3000); // ← 3 seconds is enough for live tracking
  };
  ...
  const doFetch = async (
    ...
    try {
      ...
      const parsed = data.routes.map((r, i) => {
        ...
        const traffS = isDriving
          ? leg.duration_in_traffic?.value || durS
          : durS;
        const trafficRatio = traffS / durS;
        const traffic = isDriving
          ? trafficInfo(durS, traffS)
          : { label: 'No traffic data', color: '#34A853', light: '#E6F4EA' };
        let coloredSegments = [];
        if (
          isDriving &&
          i === 0 &&
          v2FullPath &&
          v2Data.intervals?.length > 0
        ) {
          coloredSegments = buildColoredSegmentsFromV2(
            v2FullPath,
            v2Data.intervals,
          );
        } else {
          const fullPoly = stitchSteps(
            leg.steps.map((s) => ({ encodedPolyline: s.polyline.points })),
          );
          ...
          leg.steps.forEach((s) => {
            ...
            const stepRatio =
              (s.duration.value * trafficRatio) / s.duration.value;
            let color = null;
        ...
        return {
          ...
          roadPoly: stitchSteps(steps),
          overviewPoly: decode(r.overview_polyline.points),
        };
      });
      ...
      if (!resetSel && onceFetched.current && selectedSummaryRef.current) {
        const matchIdx = parsed.findIndex(
          (r) => r.summary === selectedSummaryRef.current,
        );
      ...
      if (parsed[newSelIdx]) {
        ...
        trimmedColoredSegmentsRef.current =
          parsed[newSelIdx].coloredSegments || [];
        if (navRef.current === MODE.OVERVIEW && mapRef.current) {
          setTimeout(
            () =>
              mapRef.current?.fitToCoordinates(parsed[newSelIdx].roadPoly, {
      ...
      setRoutes(parsed);
      setSelIdx(newSelIdx);
      selRef.current = newSelIdx;
      if (!isReRoute) {
        setStepIdx(0);
        stepIdxRef.current = 0;
      }
      lastFetchRef.current = nowPos;
      lastFetchPosRef.current = nowPos;
      ...
      if (isReRoute) {
        ...
        wrongDirStart.current = null;
        offRoadCount.current = 0;
        offRoadInstantRef.current = 0;
        missedTurnCountRef.current = 0;
        bearingSamplesRef.current = 0;
        routeProgressIdxRef.current = 0;
        setTurnUrgency(0);
        setDistToTurn(9999);
        distToTurnRef.current = 9999;
        selectedSummaryRef.current = null;
  ...
  const onCameraChange = useCallback(async () => {
    ...
    try {
      ...
      cameraHdgRef.current = cam.heading ?? 0;
    } catch {}
  }, []);

  const startNav = () => {
    const pos = carRef.current;
    setNavMode(MODE.NAVIGATE);
    setStepIdx(0);
    stepIdxRef.current = 0;
    setPanelOpen(false);
    setRemaining({ distM: 0, timeS: 0 });
    setWrongDir(false);
    wrongDirStart.current = null;
    offRoadCount.current = 0;
    routeProgressIdxRef.current = 0;
    ...
    setDistToTurn(9999);
    distToTurnRef.current = 9999;
    tripStatusRef.current = 'en_route';
    setTripStatus('en_route');
    notifyStatus(ambulanceId, 'en_route', ticketNo);
    ...
    if (seedRoute?.roadPoly?.length >= 2) {
      ...
      movBearingRef.current = initialBear;
      movBearingsRef.current = [initialBear, initialBear, initialBear];
      cameraHdgRef.current = initialBear;
      bearingSamplesRef.current = 0;
    } else {
      movBearingsRef.current = [];
      bearingSamplesRef.current = 0;
    }
    ...
    if (pos) {
      doFetch(pos, destRef.current, tModeRef.current, false, false);
      mapRef.current?.animateCamera(
        { center: pos, zoom: 15, heading: 0, pitch: 0 },
        { duration: 300 },
      );
      setTimeout(() => {
        const hdg = movBearingRef.current;
        cameraHdgRef.current = hdg;
        mapRef.current?.animateCamera(
          { center: pos, zoom: 18, heading: hdg, pitch: 50 },
          { duration: 900 },
        );
      }, 350);
    }
  ...
  const handleOnAction = () => {
    ...
    tripStatusRef.current = 'on_action';
    setTripStatus('on_action');
    notifyStatus(ambulanceId, 'on_action', ticketNo);
  };
  ...
  const handleAbandon = () => {
    ...
    Alert.alert(
      'Abandon Trip?',
      'Are you sure? The dispatch console will be notified immediately.',
      [
  ...
  const onMapPress = (e) => {
    ...
    selectedSummaryRef.current = null;
    if (carRef.current)
      doFetch(carRef.current, nd, tModeRef.current, false, true);
  };

  const panelH = panelAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [72, 480],
  });
  ...
  const remTime =
    remaining.timeS > 0
      ? fmtTime(remaining.timeS)
      : (activeR?.durationInTraffic ?? '');
  const remDist =
    remaining.distM > 0 ? fmtDist(remaining.distM) : (activeR?.distance ?? '');
  ...
  const turnBannerVisible =
    navMode === MODE.NAVIGATE &&
    !!curStep &&
    !reRouting &&
    !wrongDir &&
    distToTurnLabel !== '';
  const nextBarVisible =
    navMode === MODE.NAVIGATE && !!nxtStep && !reRouting && !wrongDir;

  const turnBannerTop = STATUS_BAR_H;
  const nextBarTop =
    turnBannerTop + (turnBannerVisible ? TURN_BANNER_H : 0) + 4;
  const muteButtonTop = nextBarTop + (nextBarVisible ? NEXT_BAR_H : 0) + 6;

  // ── secondary row top: below the mode bar (modeBar height ~58px + 10 top + 10 gap)
  const secondRowTop = insets.top + 10 + 60 + 8;
  ...
  return (
    <View style={s.wrap}>
      <MapView
        ...
        style={s.map}
        showsTraffic={tMode === 'driving'}
        showsUserLocation={false}
        showsMyLocationButton={navMode !== MODE.NAVIGATE}
        showsCompass
        rotateEnabled
        pitchEnabled
        onPress={onMapPress}
        ...
        mapPadding={{
          top: 0,
          right: 0,
          bottom: navMode === MODE.OVERVIEW ? 300 : 110,
          left: 0,
        }}
      ...
      >
        ...
        {navMode === MODE.NAVIGATE &&
          navAltRoutes.map((r, i) => (
            <React.Fragment key={`nav-alt-${i}`}>
              <Polyline
                coordinates={r.roadPoly}
                strokeColor='#FFFFFF'
                strokeWidth={12}
                geodesic
                zIndex={4}
        ...
        {!isWalk &&
          navMode === MODE.NAVIGATE &&
          trimmedColoredSegments.map((seg, segI) => (
            <React.Fragment key={`nav-seg-${segI}`}>
              <Polyline
                coordinates={seg.coords}
                strokeColor='#FFFFFF'
                strokeWidth={seg.isBase ? 14 : 12}
                zIndex={seg.isBase ? 6 : 10}
                lineCap='round'
                lineJoin='round'
                geodesic
              />
              <Polyline
                ...
                strokeWidth={seg.isBase ? 8 : 7}
                zIndex={seg.isBase ? 7 : 11}
                lineCap='round'
                lineJoin='round'
                geodesic
              />
        ...
        {!isWalk &&
          navMode === MODE.OVERVIEW &&
          activeR?.coloredSegments?.map((seg, segI) => (
        ...
        {navMode === MODE.NAVIGATE &&
          ...
          Math.abs(carRef.current.latitude - carPos.latitude) > 0.000005 && (
            <Polyline
              coordinates={[carRef.current, carPos]}
      ...
      {/* ═══════════════════════════════════════════════════════════════
          OVERVIEW OVERLAY — FIXED LAYOUT
          Row 1 (top): [Mode bar flex:1] [Logout button 44px]
          Row 2 (below row 1): [Traffic badge left] [Hint right]
      ═══════════════════════════════════════════════════════════════ */}
      {navMode === MODE.OVERVIEW && (
        <>
          {/* ── Row 1: Mode bar + Logout side by side ── */}
          <View style={[s.topRow, { top: insets.top + 10 }]}>
            <View style={s.modeBar}>
              {TRAVEL_MODES.map((tm) => (
                <TouchableOpacity
                  key={tm.key}
                  onPress={() => setTMode(tm.key)}
                  style={[
                    s.modeBtn,
                    tMode === tm.key && [
                      s.modeBtnOn,
                ...
                >
                  <Ionicons
                    name={tm.icon}
                    size={18}
                    color={tMode === tm.key ? '#fff' : '#666'}
                  />
                  <Text
                    style={[s.modeBtnLbl, tMode === tm.key && s.modeBtnLblOn]}
                  >
                    {tm.label}
                  </Text>
                </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              style={[s.logoutBtn, { backgroundColor: T.accent }]}
          ...
          {/* ── Row 2: Traffic badge (left) + Hint (right) — below mode bar ── */}
          {activeR && (
            <View
              style={[
                s.trafficBadge,
                { backgroundColor: activeR.traffic.color, top: secondRowTop },
              ]}
            >
              <Text style={s.trafficBadgeTxt}>{activeR.traffic.label}</Text>
            </View>
          )}
          <View style={[s.hint, { top: secondRowTop }]}>
            <Feather
              name='map-pin'
              size={11}
              color='#fff'
      ...
      {navMode === MODE.NAVIGATE && wrongDir && !reRouting && (
        <View style={[s.wrongDirBanner, { paddingTop: insets.top + 12 }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.wrongDirSub}>
              Turn around · {wrongDirDeg}° off route
            </Text>
          </View>
          <TouchableOpacity
            onPress={stopNav}
            style={s.wrongDirClose}
          >
            <Ionicons
              name='close'
              size={18}
      ...
      {navMode === MODE.NAVIGATE && (
        <View
          style={[
            s.statusBar,
            { paddingTop: insets.top + 6, paddingBottom: 6 },
          ]}
        >
          <View
            style={[
              s.statusPill,
              { backgroundColor: pillCfg.bg, borderColor: pillCfg.border },
            ]}
          >
            <View style={[s.statusDot, { backgroundColor: pillCfg.text }]} />
            <Text style={[s.statusPillTxt, { color: pillCfg.text }]}>
              {pillCfg.label}
            </Text>
          </View>
      ...
      {turnBannerVisible && (
        <View
          style={[
            s.navBanner,
            { backgroundColor: turnColor, top: turnBannerTop },
          ]}
        >
          <View
            style={[s.bnrManeuverBox, turnUrgency === 2 && s.bnrManeuverBoxNow]}
          >
            <Ionicons
              name={getManeuverIcon(
                curStep.maneuver,
                curStep.htmlInstruction || '',
              )}
      ...
      {nextBarVisible && (
        <View style={[s.nextBar, { top: nextBarTop }]}>
          ...
          <View style={s.nextDistPill}>
            <Text style={s.nextDistTxt}>{nxtStep.distance}</Text>
          </View>
        </View>
      )}
      ...
      {navMode === MODE.NAVIGATE && dropdownOpen && (
        ...
        >
          <TouchableOpacity
            style={[s.dropdownOverlay, { paddingTop: insets.top + 56 }]}
            activeOpacity={1}
            onPress={() => setDropdownOpen(false)}
          >
            <View
              style={s.dropdownCard}
            ...
            >
              ...
              {dropdownOptions.map((opt, i) => (
                <TouchableOpacity
                  ...
                  style={[
                    s.dropdownItem,
                    i < dropdownOptions.length - 1 && s.dropdownItemBorder,
                  ]}
                  onPress={opt.onPress}
                  activeOpacity={0.75}
                >
                  <Text style={[s.dropdownItemTxt, { color: opt.color }]}>
                    {opt.label}
                  </Text>
                </TouchableOpacity>
              ))}
              ...
              <View style={s.summaryRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.distTxt}>
                    {activeR.distance} · {activeR.summary}
                  </Text>
                  {activeR.endAddress ? (
                    <Text
                      style={s.addrTxt}
                ...
                <View
                  style={[
                    s.trafficDot,
                    { backgroundColor: activeR.traffic.light },
                  ]}
                >
                  <Feather
                    name='activity'
                    size={18}
                    color={activeR.traffic.color}
                  />
                </View>
              </View>
            )}
              ...
              >
                {routes.map((r, i) => {
                  ...
                  const diffLabel = isBest
                    ...
                    : diffS < 60
                      ? 'Similar'
                      : `+${Math.round(diffS / 60)} min`;
                  const diffColor = isBest
                    ? '#1E8E3E'
                    : diffS > 300
                      ? '#E53935'
                      : '#F9A825';
                  return (
                    ...
                    >
                      ...
                      >
                        <Text style={[s.routeBadgeTxt, { color: diffColor }]}>
                          {diffLabel}
                        </Text>
                      </View>
            ...
            <View style={s.stepsSec}>
              ...
              >
                {activeR.steps.map((step, i) => (
                  <View
                    key={i}
                    style={[s.stepRow, i === stepIdx && s.stepRowOn]}
                  >
                    <View
                      style={[s.stepIconBox, i === stepIdx && s.stepIconBoxOn]}
                    >
                      <Ionicons
                        name={getManeuverIcon(
                          step.maneuver,
                          step.htmlInstruction || '',
                        )}
                        size={18}
        ...
        <View style={[s.etaBar, { backgroundColor: T.mapSurface }]}>
          <View style={s.etaLeft}>
            ...
            <View style={s.etaSubRow}>
              <Text style={[s.etaDist, { color: T.textSecondary }]}>
                {remDist}
              </Text>
          ...
          <View style={s.etaRight}>
            ...
            >
              <Text style={[s.etaTrafficLbl, { color: activeR.traffic.color }]}>
                {activeR.traffic.label}
              </Text>
            </View>
            <TouchableOpacity
              style={[s.endBtn, { backgroundColor: T.accent }]}
              onPress={stopNav}
              activeOpacity={0.85}
            >
...
const s = StyleSheet.create({
  ...
  // ── FIXED: topRow wraps modeBar + logoutBtn in a horizontal row ──
  topRow: {
    position: 'absolute',
    left: 12,
    right: 12,
  ...
  // ── FIXED: modeBar uses flex:1 (fills row) instead of fixed width ──
  modeBar: {
    flex: 1,
    flexDirection: 'row',
    backgroundColor: '#fff',
  ...
  // ── FIXED: logoutBtn is in-flow (not absolutely positioned) ──
  logoutBtn: {
    width: 44,
    height: 44,
    borderRadius: 22,
    // backgroundColor set inline as T.accent
    alignItems: 'center',
    justifyContent: 'center',
    elevation: 11,
    shadowColor: '#000',
