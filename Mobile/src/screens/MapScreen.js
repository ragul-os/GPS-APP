...
const trimFromCar = (poly, pos, minStartIdx = 0) => {
  ...
  for (let i = minStartIdx; i < searchEnd; i++) { const d = dist(pos, poly[i]); if (d < minD) { minD = d; idx = i; } }
...
const snapToPolyline = (pos, poly) => {
  ...
  for (let i = 0; i < poly.length - 1; i++) {
    ...
    const t = Math.max(0, Math.min(1, ((pos.latitude - A.latitude) * (B.latitude - A.latitude) + (pos.longitude - A.longitude) * (B.longitude - A.longitude)) / ab2));
    const proj = { latitude: A.latitude + t * (B.latitude - A.latitude), longitude: A.longitude + t * (B.longitude - A.longitude) };
...
const postWithRetry = async (url, body, label) => {
  for (let attempt = 1; attempt <= POST_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      clearTimeout(timer);
      console.log(`✅ ${label} SUCCESS: ${r.status} (attempt ${attempt}/${POST_MAX_ATTEMPTS})`);
      return r;
    } catch (e) {
      clearTimeout(timer);
      const isTimeout = e?.name === 'AbortError';
      console.error(`❌ ${label} FAILED (attempt ${attempt}/${POST_MAX_ATTEMPTS}): ${isTimeout ? 'timeout after 10s' : e.message}`);
...
const ping = (ambulanceId, coords, h, spd, distM, timeS, status, stepI, totalS, dDest, ticketNo) => {
  ...
  return postWithRetry(`${WEBHOOK_URL}/webhook/abc1234`, {
    channel: 'gps', sessionId: ambulanceId, conversationId: ticketNo || ambulanceId,
    unit_id: ambulanceId, ticket_no: ticketNo || '',
...
const notifyStatus = (unitId, status, ticketNo) => {
  ...
  return postWithRetry(`${WEBHOOK_URL}/webhook/abc1234`, {
    channel: 'gps', unit_id: unitId, ticket_no: ticketNo || '', trip_status: status,
  }, 'STATUS');
};
...
export default function MapScreen({ route: navRoute, navigation }) {
  ...
  const speak = (text, opts) => { if (mutedRef.current) return; Speech.speak(text, opts); };
    ...
    if (next) {
      ...
      if (activeR && navRef.current === MODE.NAVIGATE) {
        ...
        if (currentStep) {
          ...
          const distLabel = dTurn < 50 ? '' : dTurn < 150 ? 'In 100 metres, ' : dTurn < 350 ? 'In 200 metres, ' : dTurn < 700 ? 'In 500 metres, ' : `In ${(dTurn / 1000).toFixed(1)} kilometres, `;
        ...
        if (navRef.current === MODE.NAVIGATE) {
          ...
          if (activeR) {
            ...
            for (let i = routeProgressIdxRef.current; i < searchEnd; i++) {
              const d = dist(pos, poly[i]); if (d < minD) { minD = d; bestIdx = i; }
            }
            ...
            if (!isCompletedRef.current && (currentSt === 'en_route' || currentSt === 'on_action')) {
              ...
              if (dToDest < 50) { tripStatusRef.current = 'arrived'; setTripStatus('arrived'); notifyStatus(ambulanceId, 'arrived', ticketNo); }
            }
...
const startPing = () => {
    ...
    try {
      ...
      const parsed = data.routes.map((r, i) => {
        ...
        return {
          ...
          coloredSegments, trafficSeconds: traffS,
          distanceM: leg.distance.value, distance: leg.distance.text,
      ...
      if (isReRoute) {
        ...
        setWrongDir(false); wrongDirStart.current = null;
        offRoadCount.current = 0; offRoadInstantRef.current = 0;
        missedTurnCountRef.current = 0; bearingSamplesRef.current = 0;
        routeProgressIdxRef.current = 0;
      ...
      fetchingRef.current = false; setReRouting(false); setLoading(false);
    }
  };
  ...
  const startNav = () => {
    ...
    setWrongDir(false); wrongDirStart.current = null;
    offRoadCount.current = 0; routeProgressIdxRef.current = 0;
    setTurnUrgency(0); setDistToTurn(9999); distToTurnRef.current = 9999;
    tripStatusRef.current = 'en_route'; setTripStatus('en_route');
  ...
  const handleOnAction = () => { setDropdownOpen(false); tripStatusRef.current = 'on_action'; setTripStatus('on_action'); notifyStatus(ambulanceId, 'on_action', ticketNo); };
  const handleMarkArrived = () => { setDropdownOpen(false); tripStatusRef.current = 'arrived'; setTripStatus('arrived'); notifyStatus(ambulanceId, 'arrived', ticketNo); };
  const handleCompleted = () => {
    setDropdownOpen(false); tripStatusRef.current = 'completed'; setTripStatus('completed');
    isCompletedRef.current = true; notifyStatus(ambulanceId, 'completed', ticketNo);
  ...
  return (
    <View style={s.wrap}>
      ...
      >
        ...
        {navMode === MODE.NAVIGATE && carRef.current && carPos && Math.abs(carRef.current.latitude - carPos.latitude) > 0.000005 && (
      ...
      {navMode === MODE.OVERVIEW && (
        <>
          ...
          {activeR && (
            <View style={[s.trafficBadge, { backgroundColor: activeR.traffic.color, top: secondRowTop }]}>
      ...
      {navMode === MODE.NAVIGATE && wrongDir && !reRouting && (
        <View style={[s.wrongDirBanner, { paddingTop: insets.top + 12 }]}>
          <View style={s.wrongDirIconBox}><Ionicons name='warning' size={26} color='#fff' /></View>
          <View style={{ flex: 1 }}>
      ...
      {turnBannerVisible && (
        <View style={[s.navBanner, { backgroundColor: turnColor, top: turnBannerTop }]}>
          <View style={[s.bnrManeuverBox, turnUrgency === 2 && s.bnrManeuverBoxNow]}>
            <Ionicons name={getManeuverIcon(curStep.maneuver, curStep.htmlInstruction || '')} size={turnUrgency === 2 ? 24 : 20} color='#fff' />
          </View>
      ...
      {navMode === MODE.NAVIGATE && (
        <TouchableOpacity style={[s.muteBtn, { top: muteButtonTop }]} onPress={handleMuteToggle} activeOpacity={0.8} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          <Ionicons name={muted ? 'volume-mute' : 'volume-high'} size={20} color={muted ? '#94A3B8' : '#fff'} />
        </TouchableOpacity>
      )}
            ...
            <View style={s.routeSec}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {routes.map((r, i) => {
                  ...
                  return (
                    <TouchableOpacity key={i} onPress={() => { setSelIdx(i); selRef.current = i; setStepIdx(0); stepIdxRef.current = 0; selectedSummaryRef.current = r.summary; }} style={[s.routeCard, i === selIdx && s.routeCardOn]}>
...
const s = StyleSheet.create({
  ...
  loadingChip: { position: 'absolute', alignSelf: 'center', flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(26,115,232,0.92)', borderRadius: 22, paddingHorizontal: 18, paddingVertical: 9, elevation: 10 },
  loadingTxt: { color: '#fff', fontSize: 14, fontWeight: '600' },

  recalcBanner: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1A1A2E', paddingTop: 50, paddingBottom: 16, elevation: 14, zIndex: 14 },
  recalcTxt:    { color: '#fff', fontSize: 16, fontWeight: '700', letterSpacing: 0.3 },

  wrongDirBanner: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', backgroundColor: '#D32F2F', paddingBottom: 18, paddingHorizontal: 20, elevation: 14, zIndex: 14 },
  ...
  statusBar:    { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 16, backgroundColor: 'rgba(0,0,0,0.72)', zIndex: 20, elevation: 20 },
  ...
  navBanner: { position: 'absolute', left: 0, right: 0, flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 14, elevation: 13, zIndex: 13 },
  bnrManeuverBox: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center', marginRight: 10, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.4)', flexShrink: 0 },
  bnrManeuverBoxNow: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.35)', borderWidth: 2, borderColor: '#fff' },
  bnrContent:    { flex: 1 },
  ...
  nextBar:     { position: 'absolute', left: 12, right: 12, backgroundColor: 'rgba(255,255,255,0.97)', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', elevation: 11, zIndex: 11, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 6 },
  ...
  muteBtn: { position: 'absolute', right: 14, width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.60)', alignItems: 'center', justifyContent: 'center', elevation: 12, zIndex: 19, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 6 },

  trafficBadge:    { position: 'absolute', left: 14, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, elevation: 6, zIndex: 10 },
  trafficBadgeTxt: { color: '#fff', fontWeight: '700', fontSize: 13 },

  hint:    { position: 'absolute', right: 14, flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.60)', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16, elevation: 5, zIndex: 5 },
