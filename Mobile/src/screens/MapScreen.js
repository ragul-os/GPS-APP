...
// 30s per-attempt timeout + retry with backoff for location/trip-status POSTs
...
const postWithRetry = async (url, body, label) => {
  for (let attempt = 1; attempt <= POST_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      console.error(
        `❌ ${label} FAILED (attempt ${attempt}/${POST_MAX_ATTEMPTS}): ${isTimeout ? 'timeout after 30s' : e.message}`,
      );
...
══════════════════════════════════════════════════════════════════ */}
        ...
        <View style={[s.modeBar, { top: insets.top + 10 }]}>
          {TRAVEL_MODES.map((tm) => (
            ...
            >
              ...
              />
              <Text style={[s.modeBtnLbl, tMode === tm.key && s.modeBtnLblOn]}>
                {tm.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      {navMode === MODE.OVERVIEW && activeR && (
        <View
          style={[
            s.trafficBadge,
            { backgroundColor: activeR.traffic.color, top: insets.top + 54 },
          ]}
        >
