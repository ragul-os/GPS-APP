import { Feather, Ionicons } from '@expo/vector-icons';
  ...
  const DARK_MAP_STYLE = [
    ...
    { featureType: 'road.highway',    elementType: 'geometry',        stylers: [{ color: '#44445e' }] },
    { featureType: 'road.highway',    elementType: 'geometry.stroke', stylers: [{ color: '#1a1a2e' }] },
    { featureType: 'road.local',      elementType: 'labels.text.fill',stylers: [{ color: '#555566' }] },
    ...
    { featureType: 'administrative.locality', elementType: 'labels.text.fill', stylers: [{ color: '#aaaacc' }] },
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // VEHICLE MARKER
  // The heading is baked INTO the SVG transform so Android renders it correctly.
  // We do NOT use flat={true} or Marker rotation prop — both are unreliable on Android.
  // Instead: heading is passed as prop, SVG rotates its own content around its center.
  // ─────────────────────────────────────────────────────────────────────────────
  function VehicleMarker({ heading = 0 }) {
    // SVG viewBox is 0 0 60 60, center is (30,30)
    // The arrow path points UP by default (north = 0°)
    // We rotate the inner group around center (30,30) by the heading angle
    const rotateTransform = `rotate(${heading}, 30, 30)`;
    return (
      <Svg width={60} height={60} viewBox="0 0 60 60">
        {/* White outer shadow ring */}
        <Circle cx={30} cy={30} r={28} fill="rgba(0,0,0,0.15)" />
        {/* White circle background */}
        <Circle cx={30} cy={30} r={26} fill="#FFFFFF" />
        {/* Blue circle */}
        <Circle cx={30} cy={30} r={22} fill="#4285F4" />
        {/* Arrow group — rotated by heading around center */}
        <G transform={rotateTransform}>
          {/* Arrow head — points UP (north) at 0° */}
          <Path
            d="M30 8 L39 36 L30 30 L21 36 Z"
            fill="#FFFFFF"
          />
        </G>
      </Svg>
  ...
  export default function MapScreen({ route: navRoute, navigation }) {
    ...
    useEffect(() => {
      ...
      if (!p) return;
      setRoutes([]); setTrimmed([]); setTrimmedColoredSegments([]); lastFetchRef.current = null;
      selectedSummaryRef.current = null;
      doFetch(p, destRef.current, tMode, false, true);
    }, [tMode]);

    const findProgressInFullPoly = (poly, pos, minIdx) => {
      if (!poly || poly.length < 2 || !pos) return minIdx;
      let minD = Infinity, idx = minIdx;
      const searchEnd = Math.min(poly.length, minIdx + 100);
    ...
    const trimSegmentsAtProgress = (segments, fullPoly, progressIdx, carPosition) => {
      if (!segments || !fullPoly || progressIdx <= 0) return segments;
      return segments.map(seg => {
        if (!seg.coords || seg.coords.length < 2) return seg;
        if (seg.isBase) {
          const ahead = fullPoly.slice(progressIdx);
          if (ahead.length < 2) return { ...seg, coords: [] };
          return { ...seg, coords: [carPosition, ...ahead] };
        } else {
          let firstFutureIdx = -1;
          for (let i = 0; i < seg.coords.length; i++) {
            const c = seg.coords[i];
            let foundAhead = false;
            for (let j = progressIdx; j < Math.min(fullPoly.length, progressIdx + 60); j++) {
              if (Math.abs(fullPoly[j].latitude - c.latitude) < 0.00005 &&
                  Math.abs(fullPoly[j].longitude - c.longitude) < 0.00005) {
                foundAhead = true; break;
              }
            }
            if (foundAhead) { firstFutureIdx = i; break; }
          }
          if (firstFutureIdx === -1) return { ...seg, coords: [] };
          const rem = seg.coords.slice(firstFutureIdx);
          if (rem.length < 2) return { ...seg, coords: [] };
          return { ...seg, coords: rem };
        }
      }).filter(seg => seg.coords && seg.coords.length >= 2);
    };
      ...
      locationSub.current = await Location.watchPositionAsync(
        ...
        (loc) => {
          ...
          if (dFromPrev > 3 && prevPosRef.current) {
            ...
            buf.push(rawBear); if (buf.length > 5) buf.shift();
            ...
            // In navigate mode: snap to polyline direction when on-route.
            // In overview mode: use raw GPS bearing.
            // The heading is passed into VehicleMarker as a prop so the SVG
            // rotates itself — no Marker rotation prop needed (unreliable Android).
            const activeRoute = routesRef.current[selRef.current];
            if (navRef.current === MODE.NAVIGATE && activeRoute?.roadPoly && bearingSamplesRef.current >= 2) {
              const polyBear = nearestSegmentBearing(activeRoute.roadPoly, pos);
              if (polyBear !== null) {
                const diff = angleDiff(movBearingRef.current, polyBear);
                // Trust polyline bearing when within 45° (on-route), else GPS bearing
                const finalBear = diff < 45 ? polyBear : movBearingRef.current;
                setMarkerHeading(Math.round(finalBear));
              } else {
                setMarkerHeading(Math.round(movBearingRef.current));
              }
            } else {
              setMarkerHeading(Math.round(movBearingRef.current));
            }
          ...
          const activeRoute = routesRef.current[selRef.current];
          const displayPos  =
            navRef.current === MODE.NAVIGATE && activeRoute?.roadPoly
              ? snapToPolyline(pos, activeRoute.roadPoly)
              : pos;

          carRef.current = pos;
          setCarPos({ ...displayPos });
          setReady(true);
          ...
          if (navRef.current === MODE.NAVIGATE) {
            ...
            if (activeR) {
              ...
              const newProgressIdx = findProgressInFullPoly(poly, pos, routeProgressIdxRef.current);
              if (newProgressIdx > routeProgressIdxRef.current) {
                routeProgressIdxRef.current = newProgressIdx;
              }

              const t = trimFromCar(poly, pos, routeProgressIdxRef.current);
              setTrimmed(t);

              const isWalkMode = tModeRef.current === 'walking';
              if (!isWalkMode && activeR.coloredSegments) {
                const trimmedSegs = trimSegmentsAtProgress(
                  activeR.coloredSegments, poly, routeProgressIdxRef.current, displayPos
                );
                setTrimmedColoredSegments(trimmedSegs);
                trimmedColoredSegmentsRef.current = trimmedSegs;
              }
              ...
              if (curStep?.endLocation) {
                const dToEnd    = dist(pos, curStep.endLocation);
                const offRoadNow = perpDist(pos, poly);
                if (dToEnd > 60 && dToEnd < 200 && offRoadNow > 30 && stepIdxRef.current < activeR.steps.length - 1) {
                  missedTurnCountRef.current += 1;
                  if (missedTurnCountRef.current >= 3 && now - reRouteRef.current > 6000 && !fetchingRef.current) {
                    missedTurnCountRef.current = 0; reRouteRef.current = now;
                    routeProgressIdxRef.current = 0; bearingSamplesRef.current = 0;
                    announceReroute();
                    doFetch(pos, destRef.current, tModeRef.current, true, true);
                  }
                } else { missedTurnCountRef.current = 0; }
              }

              const currentStep = activeR.steps[stepIdxRef.current];
              if (currentStep && !mutedRef.current) {
                const dTurn = distToTurnRef.current;
                if (stepIdxRef.current !== lastSpokenStepRef.current) {
                  lastSpokenStepRef.current = stepIdxRef.current;
                  spokenDistRef.current = null;
                  const distLabel =
                    dTurn < 50   ? ''
                ...
                if (dTurn < 420 && dTurn > 360 && spokenDistRef.current !== '400') {
                  spokenDistRef.current = '400';
                  Speech.speak(`In 400 metres, ${currentStep.instruction}`, { language: 'en-IN', rate: 0.92 });
                }
                if (dTurn < 85 && dTurn > 55 && spokenDistRef.current !== '80') {
                  spokenDistRef.current = '80';
                  Speech.speak(`${currentStep.instruction} ahead`, { language: 'en-IN', rate: 0.92 });
                }
                if (dTurn < 15 && spokenDistRef.current !== 'now') {
                  spokenDistRef.current = 'now';
                  Speech.speak('Turn now', { language: 'en-IN', rate: 1.0 });
                }
              }
              ...
              if (dFromPrev > 3 && bearingSamplesRef.current >= 3) {
                ...
                if (routeBear !== null) {
                  ...
                  const cooldownOk = now - reRouteRef.current > 8000;
                  if (diff > 70) {
                    setWrongDir(true); setWrongDirDeg(Math.round(diff));
                    if (!wrongDirStart.current) wrongDirStart.current = now;
                    else if (now - wrongDirStart.current > 3000 && cooldownOk && !fetchingRef.current) {
              ...
              if (!fetchingRef.current) {
                if (offRoad > 80 && now - reRouteRef.current > 6000) {
                  if (!offRoadInstantRef.current) { offRoadInstantRef.current = now; }
                  else if (now - offRoadInstantRef.current > 1500) {
                    offRoadInstantRef.current = 0; offRoadCount.current = 0; reRouteRef.current = now;
                    routeProgressIdxRef.current = 0; bearingSamplesRef.current = 0;
                    announceReroute();
                    doFetch(pos, destRef.current, tModeRef.current, true, true);
                  }
                } else { offRoadInstantRef.current = 0; }
                if (offRoad > 35 && offRoad <= 80 && now - reRouteRef.current > 6000) {
                  offRoadCount.current += 1;
                  if (offRoadCount.current >= 2) {
                    offRoadCount.current = 0; reRouteRef.current = now;
                    routeProgressIdxRef.current = 0; bearingSamplesRef.current = 0;
                    announceReroute();
                    doFetch(pos, destRef.current, tModeRef.current, true, true);
                  }
                } else if (offRoad <= 35) { offRoadCount.current = 0; }
              }

              if (lastFetchPosRef.current && !fetchingRef.current) {
                const dSince          = dist(pos, lastFetchPosRef.current);
                const timeSinceLastFetch = now - (lastPeriodicFetchRef.current || 0);
                if ((dSince >= 250 || timeSinceLastFetch >= 120000) && now - reRouteRef.current >= 6000) {
                  lastFetchPosRef.current     = pos;
                  lastPeriodicFetchRef.current = now;
            ...
            const baseZoom = spd < 15 ? 19 : spd < 40 ? 18 : spd < 80 ? 17 : 16;
            const zoom     = dTurn < 40 ? Math.min(baseZoom + 1, 20) : baseZoom - (spd > 60 ? 1 : 0);
            const isWrong  = wrongDirStart.current !== null;
            mapRef.current?.animateCamera(
              { center: displayPos, zoom, heading: ch, pitch: isWrong ? 0 : (dTurn < 40 ? 35 : 45) },
              { duration: isWrong ? 400 : (dTurn < 40 ? 180 : 320) }
            );
            return;
          }
    ...
    const startPing = () => {
      pingTimer.current = setInterval(() => {
        if (carRef.current) ping(ambulanceId, carRef.current, speedRef.current, tripStatusRef.current);
      }, 30000);
    };

    const stopAll = () => {
      clearInterval(pingTimer.current);
      clearInterval(trafficRefreshTimer.current);
      clearTimeout(navAltDismissTimer.current);
      locationSub.current?.remove();
    };

    useEffect(() => {
      if (navMode !== MODE.NAVIGATE) { clearInterval(trafficRefreshTimer.current); return; }
      trafficRefreshTimer.current = setInterval(() => {
        const pos = carRef.current;
        if (!pos || fetchingRef.current) return;
        const now = Date.now();
        if (now - lastPeriodicFetchRef.current < 90000) return;
        lastPeriodicFetchRef.current = now;
        doFetch(pos, destRef.current, tModeRef.current, false, false);
      }, 120000);
      return () => clearInterval(trafficRefreshTimer.current);
    }, [navMode]);

    const announceReroute = () => {
      const now = Date.now();
      if (now - lastRerouteAnnouncedRef.current > 8000) {
        lastRerouteAnnouncedRef.current = now;
        Speech.stop();
        if (!mutedRef.current) Speech.speak('Rerouting', { language: 'en-IN', rate: 1.0 });
      }
    };

    const showAlternateRoutesDuringNav = (parsedRoutes, currentSelIdx) => {
      if (!parsedRoutes || parsedRoutes.length < 2) return;
      const alts = parsedRoutes.filter((_, i) => i !== currentSelIdx);
      if (alts.length === 0) return;
      setNavAltRoutes(alts);
      setShowNavAlts(true);
      clearTimeout(navAltDismissTimer.current);
      navAltDismissTimer.current = setTimeout(() => {
        setShowNavAlts(false); setNavAltRoutes([]);
      }, 12000);
    };

    const switchToNavAlt = (altRoute) => {
      const pos = carRef.current;
      setShowNavAlts(false); setNavAltRoutes([]);
      clearTimeout(navAltDismissTimer.current);
      const allRoutes = routesRef.current;
      const idx = allRoutes.findIndex(r => r.summary === altRoute.summary);
      if (idx !== -1) {
        setSelIdx(idx); selRef.current = idx;
        selectedSummaryRef.current = altRoute.summary;
        routeProgressIdxRef.current = 0;
        setStepIdx(0); stepIdxRef.current = 0;
        lastSpokenStepRef.current = -1; spokenDistRef.current = null;
        if (pos) {
          setTrimmed(trimFromCar(altRoute.roadPoly, pos, 0));
          setTrimmedColoredSegments(altRoute.coloredSegments || []);
          trimmedColoredSegmentsRef.current = altRoute.coloredSegments || [];
        }
        speak(`Switching to alternate route via ${altRoute.summary}`, { language: 'en-IN', rate: 0.92 });
      }
    };
    ...
    const startNav = () => {
      ...
      setNavAltRoutes([]); setShowNavAlts(false);
      ...
      if (seedRoute?.roadPoly?.length >= 2) {
        ...
        setMarkerHeading(Math.round(initialBear));
      } else { movBearingsRef.current = []; bearingSamplesRef.current = 0; }
      if (seedRoute?.coloredSegments) {
        setTrimmedColoredSegments(seedRoute.coloredSegments);
        trimmedColoredSegmentsRef.current = seedRoute.coloredSegments;
      }
    ...
    const handleOnAction  = () => { setDropdownOpen(false); tripStatusRef.current = 'on_action';  setTripStatus('on_action');  notifyStatus(ambulanceId, 'on_action');  };
    const handleMarkArrived = () => { setDropdownOpen(false); tripStatusRef.current = 'arrived';  setTripStatus('arrived');    notifyStatus(ambulanceId, 'arrived');    };
    const handleCompleted = () => {
      setDropdownOpen(false);
      tripStatusRef.current = 'completed'; setTripStatus('completed'); isCompletedRef.current = true;
    ...
    const onMapPress = (e) => {
      ...
      setRoutes([]); setTrimmed([]); setTrimmedColoredSegments([]); lastFetchRef.current = null; selectedSummaryRef.current = null;
