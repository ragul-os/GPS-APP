  ...
  try {
    if (mediaType === 'image' || mediaType === 'video') {
      ...
      if (status !== 'granted') {
        Alert.alert(
          'Permission required',
          'Allow photo library access to save files.',
        );
        return;
      }
    }
      ...
      mediaType === 'image'
        ...
        : mediaType === 'video'
          ...
          : mediaType === 'audio'
            ...
            : 'bin';
    const safeName = (
      filename || `download_${Date.now()}.${fallbackExt}`
    ).replace(/[^a-zA-Z0-9._-]/g, '_');
    const localUri = `${FileSystem.cacheDirectory}${safeName}`;
    const { uri: localPath, status: httpStatus } =
    ...
    if (mediaType === 'image' || mediaType === 'video') {
      ...
      Alert.alert(
        'Saved ✓',
        `${mediaType === 'image' ? 'Photo' : 'Video'} saved to your gallery.`,
      );
    } else {
...
export default function ChatScreen({
  ...
  const loadMessages = async () => {
    // Kick off the sync loop in parallel with history fetch so incoming messages
    // start streaming as early as possible (was previously serialised after history).
    startSync();
    try {
  ...
  const startSync = useCallback(async () => {
    let since = null;
    // Scope the sync to ONLY the currently-open room. Without this, every initial
    // sync pulls full state for every joined room — that delay was blocking
    // incoming messages from being delivered to the chat for several seconds.
    const roomFilter = {
      room: {
        rooms: [roomIdRef.current],
    ...
    try {
      const initial = await syncMatrix(
        session.accessToken,
        null,
        0,
        roomFilter,
      ...
      const pinEvent = joinData?.state?.events?.find(
        (e) => e.type === 'm.room.pinned_events',
      );
    ...
    while (syncActive.current) {
      try {
        ...
        if (roomData) {
          const pinEvent = roomData.state?.events?.find(
            (e) => e.type === 'm.room.pinned_events',
          );
          if (pinEvent) setPinnedEvents(pinEvent.content?.pinned || []);
            ...
            events
              ...
              .forEach((e) => {
                ...
                if (relate?.rel_type === 'm.annotation' && relate.event_id) {
                  ...
                  if (!newReactions[eid][key].includes(e.sender))
                    newReactions[eid][key].push(e.sender);
                }
              });
  ...
  function parseEvent(event) {
    ...
    return {
      ...
      geo:
        content.geo_uri || content['org.matrix.msc3488.location']?.uri || null,
  ...
  async function handleSend() {
    ...
    try {
      ...
      // Keep the optimistic bubble visible until the sync loop replaces it via
      // its txnId-based dedupe; removing it here causes a brief gap where the
      // sent message disappears, which felt like "slow delivery".
    } catch (err) {
  ...
  const renderMessage = ({ item: msg, index }) => {
    ...
    const isMediaMsg =
      (msg.msgtype === 'm.image' || msg.msgtype === 'm.video') &&
      !!msg.mediaUrl;
    ...
    return (
      <View style={[st.msgRow, msg.isMe && st.msgRowMe]}>
          ...
          <View style={st.avatarWrap}>
            {showMeta ? (
              <View style={[st.avatar, { backgroundColor: accentFaint }]}>
                <Text style={[st.avatarTxt, { color: accent }]}>
        ...
        >
          ...
          {replyMsg && (
            <View style={isMediaMsg && st.mediaPad}>
              <TouchableOpacity
                style={[
                  st.replyBubble,
                  msg.isMe && st.replyBubbleMe,
                  {
                    borderLeftColor: msg.isMe
              ...
              >
                <Text
                  style={[
                    st.replySender,
                    { color: msg.isMe ? 'rgba(255,255,255,0.9)' : accent },
                  ]}
                >
                  {replyMsg.senderName}
                </Text>
                <Text
                  style={[
                    st.replyBody,
                    {
                      color: msg.isMe ? 'rgba(255,255,255,0.7)' : textSecondary,
                    },
                  ]}
                  numberOfLines={1}
                >
          ...
          {msg.msgtype === 'm.text' && (
            <Text
              style={[st.msgText, { color: msg.isMe ? '#fff' : textPrimary }]}
            >
              {msg.body}
            </Text>
          )}
              ...
              style={[
                st.fileCard,
                {
                  backgroundColor: msg.isMe
                    ...
                    : accentFaint,
                  borderColor: msg.isMe ? 'rgba(255,255,255,0.2)' : accentMid,
                },
              ]}
            >
              <View
                style={[
                  st.fileCardIconCircle,
                  {
                    backgroundColor: msg.isMe
              ...
              >
                <Feather
                  name={getFileIconName(msg.mimeType)}
                  size={22}
                  color={msg.isMe ? '#fff' : accent}
                />
              </View>
              <View style={st.fileCardInfo}>
                <Text
                  style={[
                    st.fileCardName,
                    { color: msg.isMe ? '#fff' : textPrimary },
                  ]}
                  numberOfLines={2}
                >
                ...
                <Text
                  style={[
                    st.fileCardMeta,
                    { color: msg.isMe ? 'rgba(255,255,255,0.6)' : textMuted },
                  ]}
                >
                  {msg.mimeType?.split('/').pop()?.toUpperCase() || 'FILE'} ·{' '}
                  {fmtFileSize(msg.fileSize)}
                </Text>
              </View>
              {msg.mediaUrl && (
                <TouchableOpacity
          ...
          {msg.msgtype === 'm.location' && (
            <View style={[st.locBubble, { backgroundColor: accentFaint }]}>
              <MapView
                ...
                initialRegion={{
                  latitude:
                    parseFloat(msg.geo?.split(':')[1]?.split(',')[0]) || 0,
                  longitude:
                    parseFloat(msg.geo?.split(':')[1]?.split(',')[1]) || 0,
                  latitudeDelta: 0.01,
                  longitudeDelta: 0.01,
                }}
              >
          ...
          <View style={[st.msgFooter, isMediaMsg && st.msgFooterMedia]}>
                ...
                {Object.entries(msgReactions).map(([emoji, users]) => (
                  ...
                  >
                    ...
                    <Text
                      style={[
                        st.reactionCount,
                        users.includes(session.userId) && { color: accent },
                      ]}
                    >
                      {users.length}
                    </Text>
            ...
            <Text
              style={[
                st.msgTime,
                { color: msg.isMe ? 'rgba(255,255,255,0.65)' : textMuted },
              ]}
            >
              {time}
            </Text>
      ...
      try {
        ...
        if (json.success && json.data) {
          ...
          if (match) {
            ...
            const rawCache =
              (await AsyncStorage.getItem('TICKET_NAMES')) || '{}';
    ...
    return (
      <View style={[st.loader, { backgroundColor: bg }]}>
        <ActivityIndicator
          ...
          color={accent}
        />
        <Text style={[st.loaderTxt, { color: textSecondary }]}>
          Loading messages…
        </Text>
      </View>
    );
  ...
  const isTicketRoom = currentRoomName
    ?.trim()
    ?.toLowerCase()
    ?.includes('ticket-');
  const displayTitle =
    patientData?.name || // patient name if found
    currentRoomName ||
    'Incident Chat'; // ← always show room name as fallback
  ...
  return (
    <KeyboardAvoidingView
      style={[st.flex, { backgroundColor: bg }]}
    ...
    >
      ...
      {pinnedEvents.length > 0 && (
        ...
        >
          ...
          />
          <Text
            style={[st.pinnedTxt, { color: textPrimary }]}
            numberOfLines={1}
          >
            {pinnedEvents.length} Pinned{' '}
            {pinnedEvents.length === 1 ? 'Message' : 'Messages'}
          </Text>
          <TouchableOpacity
            onPress={() => {
        ...
        ListEmptyComponent={
          <View style={st.emptyWrap}>
            <Ionicons
              ...
              color={accentFaint}
            />
            <Text style={[st.emptyTxt, { color: textSecondary }]}>
              No messages yet
            </Text>
            <Text style={[st.emptySub, { color: textMuted }]}>
      ...
      {isRecording && (
        <View
          style={[
            st.recordingBar,
            { backgroundColor: accentSoft, borderTopColor: accentMid },
          ]}
        >
          <View style={st.recordingDot} />
          <Text style={[st.recordingTxt, { color: accent }]}>
            Recording… tap stop to send
          </Text>
          <TouchableOpacity
            onPress={cancelRecording}
            style={st.cancelRecBtn}
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
          >
            <Ionicons
              name='close'
      ...
      {replyTo && (
        ...
        >
          <View style={{ flex: 1 }}>
            <Text style={[st.replyTitle, { color: accent }]}>
              Replying to {replyTo.senderName}
            </Text>
            <Text
              style={[st.replyText, { color: textSecondary }]}
              numberOfLines={1}
            >
              {replyTo.body}
            </Text>
        ...
        >
          {roomMembers
            ...
            .map((m) => (
              ...
              >
                <View
                  style={[st.mentionAvatar, { backgroundColor: accentFaint }]}
                >
                  <Text style={[st.mentionAvatarTxt, { color: accent }]}>
                    {m.displayName.charAt(0).toUpperCase()}
                  </Text>
                </View>
                <Text style={[st.mentionName, { color: textPrimary }]}>
                  {m.displayName}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={st.attachGrid}>
            {ATTACH_ITEMS.map((item) => (
              ...
              >
                ...
                <Text style={[st.attachItemLabel, { color: textSecondary }]}>
                  {item.label}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      >
        ...
        >
          <View style={[camStyle.card, { backgroundColor: surface }]}>
            ...
            <Text style={[camStyle.title, { color: textPrimary }]}>
              Open Camera
            </Text>
            <Text style={[camStyle.subtitle, { color: textMuted }]}>
              What would you like to capture?
            </Text>
            <View style={camStyle.row}>
              <TouchableOpacity
                style={camStyle.option}
              ...
              >
                ...
                <Text style={[camStyle.optionLabel, { color: textPrimary }]}>
                  Photo
                </Text>
                <Text style={[camStyle.optionSub, { color: textMuted }]}>
                  Take a picture
                </Text>
              </TouchableOpacity>
              ...
              >
                <View
                  style={[camStyle.optionIcon, { backgroundColor: '#FEE2E2' }]}
                >
                  <Feather
                    name='video'
                    size={28}
            ...
            <TouchableOpacity
              style={[camStyle.cancelBtn, { backgroundColor: surfaceAlt2 }]}
              onPress={() => setShowCameraChoiceModal(false)}
            >
              <Text style={[camStyle.cancelTxt, { color: textSecondary }]}>
                Cancel
              </Text>
            </TouchableOpacity>
          </View>
      ...
      >
        <View style={st.locModalBg}>
          <View style={[st.locModalCard, { backgroundColor: surface }]}>
            ...
            <Text style={[st.locModalTitle, { color: textPrimary }]}>
            ...
            {[
              ...
              <TouchableOpacity
                key={i}
                style={[st.locOptionBtn, { borderColor: border }]}
                onPress={opt.onPress}
              >
                <View
                  style={[
                ...
                >
                  {opt.ionIcon ? (
                    ...
                    <Text
                      style={{ color: opt.borderColor, fontWeight: 'bold' }}
                    >
                      {opt.icon}
                    </Text>
                  )}
                </View>
                <Text style={[st.locOptionTxt, { color: textSecondary }]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <Modal
        visible={showLiveDurationModal}
        transparent
        animationType='fade'
        onRequestClose={() => setShowLiveDurationModal(false)}
      >
        <View style={st.locModalBg}>
          <View style={[st.locModalCard, { backgroundColor: surface }]}>
            <Text style={[st.locModalTitle, { color: textPrimary }]}>
              Share live location for how long?
            </Text>
            {[
              ...
              <TouchableOpacity
                key={dur.label}
                style={[st.locOptionBtn, { borderColor: border }]}
                onPress={() => handleShareLiveLocation(dur.val)}
              >
                <Text style={[st.locOptionTxtCenter, { color: textSecondary }]}>
                  {dur.label}
                </Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity
              style={[st.locOptionBtn, { marginTop: 10, borderColor: border }]}
              onPress={() => setShowLiveDurationModal(false)}
            >
              <Text style={[st.locOptionTxtCenter, { color: '#EF4444' }]}>
                Cancel
              </Text>
            </TouchableOpacity>
          </View>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <View style={st.pinToolbar}>
            <TouchableOpacity
              style={[st.pinCancelBtn, { backgroundColor: textMuted }]}
              onPress={() => setShowPinModal(false)}
            >
              <Text style={{ color: '#fff' }}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[st.pinConfirmBtn, { backgroundColor: accent }]}
              onPress={handleSharePinLocation}
            >
      ...
      >
        ...
        >
          <View style={[st.emojiContent, { backgroundColor: surface }]}>
            <Text style={[st.emojiTitle, { color: textPrimary }]}>
              Select Emoji
            </Text>
            ...
            <View style={st.whatsappActionList}>
              ...
              >
                <Ionicons
                  ...
                  color={accent}
                />
                <Text style={[st.whatsappActionText, { color: textPrimary }]}>
                  Reply
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    const nextPins = await pinMessage(
                      session.accessToken,
                      roomId,
                      selectedMessage.id,
                    );
              ...
              >
                <Ionicons
                  name={
                    pinnedEvents.includes(selectedMessage?.id)
                      ? 'pin-off'
                      : 'pin'
                  }
                  size={24}
    ...
    <View style={ast.audioBubble}>
      <View style={ast.audioTopRow}>
        <TouchableOpacity
          style={[
            ast.audioPlayBtn,
            { backgroundColor: playBg, borderColor: trackActive + '44' },
          ]}
          onPress={onToggle}
          activeOpacity={0.7}
        >
        ...
        {onDownload && (
          <TouchableOpacity
            onPress={onDownload}
            style={ast.audioDownloadBtn}
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
          >
            <Ionicons
              name='download-outline'
              size={18}
              color={isMe ? 'rgba(255,255,255,0.85)' : accent}
            />
          </TouchableOpacity>
        )}
      </View>
      <View style={ast.audioMetaRow}>
        <Text
          style={[
            ast.audioTimeTxt,
            { color: isMe ? 'rgba(255,255,255,0.65)' : '#64748B' },
          ]}
        >
