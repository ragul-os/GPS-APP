...
// ── Fetch role from EMS backend (public.ems_users) ───────────────────────────
async function fetchUserRole(username) {
  const res = await fetch(
    `${API_BASE_URL}/api/user-role/${encodeURIComponent(username.trim().toLowerCase())}`,
  );
  const data = await res.json();
  if (!res.ok) {
    throw new Error(
      data.error || 'Role lookup failed. Contact your administrator.',
    );
  }
  return data.role; // 'agent' | 'dispatcher' | 'admin'
}
...
export function AuthProvider({ children }) {
  ...
  // callbacks = { onUserNotFound, onUserCreated, onLoginSuccess, onRoleFetch }
  // Role is no longer passed from the UI — it is fetched from the backend.
  async function login(username, password, callbacks = {}) {
    const { onUserNotFound, onUserCreated, onLoginSuccess, onRoleFetch } =
      callbacks;
    ...
    try {
      // ── Step 1: Try Matrix login directly ──────────────────────────────
      ...
      if (
        ...
        err.errcode === 'M_USER_DOES_NOT_EXIST'
      ) {
        ...
        try {
          ...
          console.error(
            '[Auth] Creation or post-creation login failed:',
            createErr,
          );
    ...
    // ── Step 4: Fetch role from EMS backend ─────────────────────────────
    onRoleFetch?.();
    console.log('[Auth] Fetching role from backend for:', username);
    const role = await fetchUserRole(username); // throws if not found
    console.log('[Auth] Role received:', role);
    ...
    const user = {
      ...
      role, // 'agent' | 'dispatcher' | 'admin'  — backend-assigned
    };

    console.log('[Auth] Session stored:', user.userId, '| role:', user.role);
