...
export default function LoginPage() {
  ...
  // Redirect if already logged in → go to their role's home
  useEffect(() => {
    if (dispatcher) navigate(roleHome(dispatcher.role), { replace: true });
  }, [dispatcher, navigate]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const user = await login(username.trim(), password, {
        // No role passed — backend detects it automatically
        onUserNotFound: () => {
          setStep(2);
          setStatusMsg('Registering new account...');
        },
        onUserCreated: () => {
        ...
        onLoginSuccess: () => {
          setStep(3);
          setStatusMsg('Credentials verified. Fetching access level...');
        },
        onRoleFetch: () => {
          setStatusMsg('Access level detected. Loading control system...');
        },
      });
      // Redirect to the role's home — role is returned from backend
      setTimeout(() => navigate(roleHome(user.role), { replace: true }), 900);
    } catch (err) {
    ...
    <div style={s.page}>
      ...
      <div style={s.topBar}>
        <div style={s.topBarLeft}>
          ...
          />
          <span style={s.topBarTitle}>Emergency Management System</span>
        </div>
      ...
      <div style={s.center}>
        <div style={s.centerCol}>
          ...
          <div style={s.titleBlock}>
            ...
            />
            <div>
              <div style={s.titleMain}>Emergency Management SYSTEM</div>
              <div style={s.titleSub}>
                Secure Access Portal — Authorised Personnel Only
              </div>
            </div>
          </div>
          ...
          <div style={s.card}>
            ...
            <div style={s.cardHeader}>
              <div style={s.cardHeaderLabel}>SYSTEM LOGIN</div>
              <div style={s.cardHeaderSub}>
                Enter your credentials — access level is detected automatically
              </div>
            </div>
            ...
            {error && (
              <div style={s.errorBanner}>
                <WarningOutlined
                  style={{ marginRight: 8, fontSize: 13, color: '#ef4444' }}
                />
                {error}
              </div>
            )}
            ...
            <div style={s.fields}>
              ...
              <div style={s.fieldGroup}>
                ...
                <div style={s.inputRow}>
                  <span style={s.inputIcon}>
                    <LockOutlined style={{ color: '#475569' }} />
                  </span>
                  <input
                    ref={passwordRef}
      ...
      <div style={s.bottomBar}>
        <span>Emergency Management SYSTEM — RESTRICTED ACCESS</span>
        <span>v3.1.4</span>
      </div>
