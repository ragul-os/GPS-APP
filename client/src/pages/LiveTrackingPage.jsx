...
import { closeTicketEvent, getTicketEvent, unitTicketEvent } from '../services/ticketEventsApi';
...
const UCFG = {
  ambulance: { icon: <MedicineBoxOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, label: 'Ambulance', color: '#4CAF50' },
  fire: { icon: <FireOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, label: 'Fire Engine', color: '#FB8C00' },
  police: { icon: <SafetyOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, label: 'Police Unit', color: '#1E88E5' },
  rescue: { icon: <AlertOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, label: 'Rescue', color: '#8E24AA' },
...
const TICKET_STATUS_CFG = {
  pending: { label: 'Pending', icon: <ClockCircleOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, color: '#F9A825', bg: 'rgba(249,168,37,.15)' },
  dispatched: { label: 'Dispatched', icon: <NodeIndexOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, color: '#1A73E8', bg: 'rgba(26,115,232,.15)' },
  completed: { label: 'Completed', icon: <CheckCircleOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, color: '#34A853', bg: 'rgba(52,168,83,.15)' },
  rejected: { label: 'Rejected', icon: <CloseCircleOutlined style={{ fontSize: '16px', verticalAlign: 'middle' }} />, color: '#E53935', bg: 'rgba(229,57,53,.15)' },
};
...
function haversine(la1, lo1, la2, lo2) {
  const R = 6371000, φ1 = (la1 * Math.PI) / 180, φ2 = (la2 * Math.PI) / 180,
    dφ = ((la2 - la1) * Math.PI) / 180, dλ = ((lo2 - lo1) * Math.PI) / 180,
...
function trafficLabel(r) {
  if (r < 1.1) return { label: 'Free flow', color: '#34A853', icon: <CheckCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#52c41a' }} />, short: 'Free' };
  if (r < 1.3) return { label: 'Moderate', color: '#F9A825', icon: <ClockCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#faad14' }} />, short: 'Mod' };
  if (r < 1.6) return { label: 'Heavy', color: '#E53935', icon: <ExclamationCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#ff4d4f' }} />, short: 'Heavy' };
  return { label: 'Stop & Go', color: '#B71C1C', icon: <ExclamationCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#ff4d4f' }} />, short: 'Stop&Go' };
}
...
function stitch(steps) {
  ...
  for (const st of steps) {
    ...
    if (o.length && pts.length) {
      const l = o[o.length - 1], f = pts[0];
      if (Math.abs(l.lat - f.lat) < 1e-6 && Math.abs(l.lng - f.lng) < 1e-6) o.push(...pts.slice(1));
      else o.push(...pts);
    } else o.push(...pts);
  }
  return o;
...
async function fetchRoutesV2(originLat, originLng, destLat, destLng) {
  try {
    ...
    const res = await fetch(GOOGLE_ROUTES_COMPUTE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Goog-Api-Key': GOOGLE_MAPS_API_KEY, 'X-Goog-FieldMask': ['routes.distanceMeters', 'routes.duration', 'routes.staticDuration', 'routes.polyline.encodedPolyline', 'routes.travelAdvisory.speedReadingIntervals'].join(',') },
  ...
  } catch (e) { console.error('[RoutesV2] Fetch error:', e); return null; }
}
...
function updateAgentTicketStatus(ticketId, status) {
  ...
  if (idx !== -1) { tickets[idx].status = status; saveAgentTickets(tickets); }
}
function updateAlertHistoryStatus(alertId, status) {
  ...
  if (idx !== -1) { history[idx].status = status; localStorage.setItem('alertHistory', JSON.stringify(history)); window.dispatchEvent(new Event('alertHistoryChange')); }
}
...
function TicketDetailsOverlay({ alertObj, agentTicket, ticketStatus, dispatchedUnits, unitStatuses, unitTypes, activeUnitId, onSwitchUnit }) {
  ...
  const ticketNo = agentTicket?.id || alertObj?.agentTicketId || alertObj?.id || '—';
  ...
  return (
    <div style={ns.ticketOverlay}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginBottom: collapsed ? 0 : 8 }}>
        ...
        <button onClick={() => setCollapsed((v) => !v)} style={ns.collapseBtn}>
          {collapsed ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}
        </button>
      </div>
        ...
        <>
              ...
              {dispatchedUnits.length > 0 && (
                <>
                  ...
                  {dispatchedUnits.map((uid) => {
                    const st = unitStatuses[uid] || 'dispatched', stc = UNIT_ST_COLOR[st] || '#8B949E', sti = UNIT_ST_ICON[st] || <MedicineBoxOutlined />;
                    return (
                      <div key={uid} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid rgba(48,54,61,0.3)' }}>
                        ...
                        <span style={{ fontSize: 8, fontWeight: 700, padding: '1px 5px', borderRadius: 5, background: `${stc}18`, color: stc, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4 }}>{sti} {st}</span>
                      </div>
                    );
                  })}
                  {completedCount === dispatchedUnits.length && dispatchedUnits.length > 0 && (
                    <div style={{ marginTop: 6, padding: '5px 8px', borderRadius: 7, background: 'rgba(52,168,83,0.12)', border: '1px solid rgba(52,168,83,0.25)', fontSize: 9, fontWeight: 700, color: '#34A853', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                      <CheckCircleOutlined style={{ fontSize: '10px' }} /> All units completed
                    </div>
                  )}
                </>
              )}
                ...
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid rgba(48,54,61,0.5)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {dispatchedUnits.map((uid) => {
                      ...
                      return (
                        <button key={uid} onClick={() => onSwitchUnit && onSwitchUnit(uid)} style={{ ...ns.switcherBtn, background: isActive ? `${ucfg.color}22` : 'rgba(30,37,46,0.7)', border: `1.5px solid ${isActive ? ucfg.color : 'rgba(48,54,61,0.7)'}`, color: isActive ? ucfg.color : '#8B949E', boxShadow: isActive ? `0 0 10px ${ucfg.color}44` : 'none' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 1, minWidth: 0, flex: 1 }}>
                            ...
                            <span style={{ fontSize: 7, color: stc, fontWeight: 700, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 3 }}>{UNIT_ST_ICON[st]} {st}</span>
                          </div>
              ...
              {dispatchedUnits.length === 1 && (
                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 7, padding: '6px 9px', borderRadius: 8, background: 'rgba(26,115,232,0.08)', border: '1px solid rgba(26,115,232,0.2)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 7, color: '#82B4FF', fontWeight: 700, marginTop: 1, display: 'flex', alignItems: 'center', gap: 3 }}>
                      <CheckCircleFilled style={{ color: '#ff4d4f', fontSize: '6px' }} /> LIVE TRACKING
                    </div>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'ticket' && (
            <div style={{ marginTop: 8 }}>
              ...
              {[
                { label: 'Phone', val: phone, icon: <PhoneOutlined style={{ fontSize: '10px' }} /> },
                { label: 'Address', val: address, icon: <EnvironmentOutlined style={{ fontSize: '10px' }} /> },
                dest?.latitude && { label: 'Coords', val: `${dest.latitude.toFixed(5)}, ${dest.longitude.toFixed(5)}`, icon: <AimOutlined style={{ fontSize: '10px' }} /> },
                answers.f4 && { label: 'Type', val: answers.f4, icon: <PushpinOutlined style={{ fontSize: '10px' }} /> },
                notes && { label: 'Notes', val: notes, icon: <FileTextOutlined style={{ fontSize: '10px' }} /> },
              ].filter(Boolean).map((item) => (
                <div key={item.label} style={ns.detRow}>
                  <span style={ns.detLbl}>{item.icon} {item.label}</span>
                  <span style={{ ...ns.detVal, fontFamily: item.label === 'Coords' ? 'JetBrains Mono, monospace' : 'inherit', fontSize: item.label === 'Coords' ? 9 : 10, color: item.label === 'Coords' ? '#1A73E8' : '#E6EDF3' }}>{item.val}</span>
                </div>
              ))}
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 8, background: stCfg.bg, border: `1px solid ${stCfg.color}40` }}>
                  ...
                  {ticketStatus === 'completed' && <span style={{ fontSize: 8, fontWeight: 800, color: '#0e100f', background: 'rgba(52,168,83,0.2)', padding: '2px 6px', borderRadius: 4, border: '1px solid rgba(52,168,83,0.3)' }}>CLOSED</span>}
                </div>
              </div>
              <div style={{ marginTop: 8, padding: '5px 8px', borderRadius: 7, background: 'rgba(26,115,232,0.06)', border: '1px solid rgba(26,115,232,0.15)' }}>
...
function MiniMapOverlay({ alertObj, dispatchedUnits, unitLocations, unitTypes, onUnitClick, activeUnitId }) {
  ...
  useEffect(() => {
    if (collapsed) {
      ...
      miniMkrsRef.current = {}; miniMapObj.current = null; hasFitOnce.current = false;
    }
  }, [collapsed]);

  useEffect(() => {
    if (collapsed || !miniMapRef.current || miniMapObj.current || !window.google?.maps) return;
    const center = alertObj?.destination?.latitude ? { lat: alertObj.destination.latitude, lng: alertObj.destination.longitude } : { lat: 11.0168, lng: 76.9558 };
    miniMapObj.current = new window.google.maps.Map(miniMapRef.current, { center, zoom: 13, styles: DARK_MAP_STYLES, mapTypeControl: false, streetViewControl: false, fullscreenControl: false, zoomControl: true, gestureHandling: 'greedy' });
    if (alertObj?.destination?.latitude) {
      const { latitude: dlat, longitude: dlng } = alertObj.destination;
      new window.google.maps.Marker({ position: { lat: dlat, lng: dlng }, map: miniMapObj.current, zIndex: 90, icon: { url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="28" height="36" viewBox="0 0 28 36"><path d="M14 0C6 0 0 6 0 14C0 25 14 36 14 36C14 36 28 25 28 14C28 6 22 0 14 0Z" fill="#E53935" stroke="white" stroke-width="2"/><circle cx="14" cy="14" r="5" fill="white"/></svg>`), scaledSize: new window.google.maps.Size(28, 36), anchor: new window.google.maps.Point(14, 36) } });
    }
  }, [collapsed, alertObj]);
  ...
  useEffect(() => {
    ...
    let hasLoc = false;
    if (alertObj?.destination?.latitude) bounds.extend({ lat: alertObj.destination.latitude, lng: alertObj.destination.longitude });
    dispatchedUnits.forEach((uid) => {
      ...
      const type = unitTypes[uid] || 'ambulance', ucfg = UCFG[type] || UCFG.ambulance;
      const isActive = uid === activeUnitId, outerSize = isActive ? 44 : 30, innerR = isActive ? 14 : 9, ringR = isActive ? 20 : 0, sw = isActive ? 3 : 1.5, op = isActive ? 1 : 0.65, zIdx = isActive ? 110 : 100;
      const svg = isActive
      ...
      } else { miniMkrsRef.current[uid].setPosition({ lat, lng }); miniMkrsRef.current[uid].setIcon(icon); miniMkrsRef.current[uid].setZIndex(zIdx); }
    });
    if (!hasFitOnce.current && hasLoc && !bounds.isEmpty()) { try { miniMapObj.current.fitBounds(bounds, { padding: 32 }); hasFitOnce.current = true; } catch (_) {} }
...
function TrackingLogPanel({ tLogs, dispatchedUnits, activeUnitId, unitTypes }) {
  ...
  useEffect(() => { if (logBoxRef.current) logBoxRef.current.scrollTop = logBoxRef.current.scrollHeight; }, [tLogs]);
  ...
  return (
    <div style={{ ...s.section, flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 8, background: '#0D1117', borderRadius: 8, padding: 3, border: '1px solid #30363D' }}>
        {[{ key: 'all', label: 'All Units', icon: <ApartmentOutlined style={{ fontSize: '10px' }} /> }, { key: 'active', label: 'Active Unit', icon: <AimOutlined style={{ fontSize: '10px' }} /> }, { key: 'custom', label: 'Custom', icon: <SettingOutlined style={{ fontSize: '10px' }} /> }].map(({ key, label, icon }) => (
      ...
      {logTab === 'custom' && (
        <div style={{ marginBottom: 8 }}>
          <button onClick={() => setCustomOpen((v) => !v)} style={{ width: '100%', padding: '5px 10px', borderRadius: 7, border: '1px solid rgba(26,115,232,0.3)', background: 'rgba(26,115,232,0.08)', color: '#82B4FF', fontFamily: 'Sora, sans-serif', fontSize: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span><SettingOutlined style={{ fontSize: '10px', marginRight: 4 }} /> Select Units ({customUnits.size === 0 ? 'all' : customUnits.size} selected)</span>
            <span>{customOpen ? <UpOutlined style={{ fontSize: '9px' }} /> : <DownOutlined style={{ fontSize: '9px' }} />}</span>
          </button>
          {customOpen && (
            <div style={{ marginTop: 6, background: '#0D1117', border: '1px solid #30363D', borderRadius: 8, padding: '6px 8px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {dispatchedUnits.map((uid) => {
                ...
                return (
                  <label key={uid} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '4px 6px', borderRadius: 6, background: checked ? `${ucfg.color}11` : 'transparent', border: `1px solid ${checked ? ucfg.color + '44' : 'transparent'}` }}>
                    <div style={{ width: 14, height: 14, borderRadius: 3, border: `2px solid ${checked ? ucfg.color : '#30363D'}`, background: checked ? ucfg.color : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{checked && <span style={{ fontSize: 9, color: 'white', lineHeight: 1 }}>✓</span>}</div>
              ...
              <div style={{ display: 'flex', gap: 5, marginTop: 4, paddingTop: 4, borderTop: '1px solid #30363D' }}>
                <button onClick={() => setCustomUnits(new Set(dispatchedUnits))} style={{ flex: 1, padding: '3px 0', borderRadius: 5, border: '1px solid rgba(52,168,83,0.3)', background: 'rgba(52,168,83,0.08)', color: '#34A853', fontSize: 9, fontWeight: 700, cursor: 'pointer', fontFamily: 'Sora, sans-serif' }}>All</button>
                <button onClick={() => setCustomUnits(new Set())} style={{ flex: 1, padding: '3px 0', borderRadius: 5, border: '1px solid rgba(139,148,158,0.3)', background: 'rgba(139,148,158,0.08)', color: '#8B949E', fontSize: 9, fontWeight: 700, cursor: 'pointer', fontFamily: 'Sora, sans-serif' }}>Clear</button>
              </div>
            </div>
          )}
        </div>
      ...
      {logTab === 'active' && activeUnitId && (
        <div style={{ marginBottom: 6, padding: '4px 8px', borderRadius: 6, background: 'rgba(26,115,232,0.08)', border: '1px solid rgba(26,115,232,0.2)', fontSize: 9, fontWeight: 700, color: '#82B4FF', display: 'flex', alignItems: 'center', gap: 5 }}>
          <span><AimOutlined style={{ fontSize: '10px' }} /> Filtering:</span>
          ...
          filteredLogs.map((l, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 2, lineHeight: 1.5 }}>
                {l.unitId && <span style={{ fontSize: 7, fontWeight: 800, flexShrink: 0, marginTop: 2, padding: '1px 4px', borderRadius: 3, background: `${(UCFG[unitTypes[l.unitId] || 'ambulance'] || UCFG.ambulance).color}22`, color: (UCFG[unitTypes[l.unitId] || 'ambulance'] || UCFG.ambulance).color, fontFamily: 'JetBrains Mono, monospace', border: `1px solid ${(UCFG[unitTypes[l.unitId] || 'ambulance'] || UCFG.ambulance).color}44`, maxWidth: 52, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.unitId.slice(0, 6)}</span>}
                <span style={{ color: logColors[l.type] || '#8B949E', flex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>{icons[l.type] || icons.info} {l.msg}</span>
              </div>
            );
          })
        )}
...
export default function LiveTrackingPage() {
  ...
  const alertObj =
    ...
    JSON.parse(localStorage.getItem('alertHistory') || '[]').find((a) => a.id === id || a.agentTicketId === id) ||
    null;
  ...
  const tryCloseTicket = useCallback(async (callerUnitId) => {
    ...
    const allDone = units.every((uid) => completedUnitsSetRef.current.has(uid));
    ...
    if (!allDone) {
      const pending = units.filter((uid) => !completedUnitsSetRef.current.has(uid));
      addTLog(`Unit ${callerUnitId} completed — waiting for: ${pending.join(', ')}`, 'info', callerUnitId);
      return;
    }
    ...
    addTLog(`All ${units.length} unit(s) completed → ticket auto-completed`, 'ok');
    ...
    try {
      for (const uid of units) {
        await unitTicketEvent(ticketId, {
          ...
          if (err?.response?.status !== 409) {
            console.warn(`[tryCloseTicket] unitTicketEvent failed for ${uid}:`, err.message);
          }
        });
      }
      ...
      addTLog(`closeTicketEvent failed: ${err?.response?.data?.error || err.message}`, 'warn');
    }
  }, [id, addTLog]);
  ...
  const fetchRoute = useCallback(async (lat, lng, uid) => {
    ...
    try {
      ...
      if (routes?.length) {
        parsed = routes.map((r, i) => { const leg = r.legs[0]; return { fp: stitch(leg.steps), steps: leg.steps, durationS: leg.duration.value, trafficS: leg.duration_in_traffic?.value ?? leg.duration.value, distanceM: leg.distance.value, summary: r.summary || `Route ${i + 1}` }; }).sort((a, b) => a.trafficS - b.trafficS);
        routeDataRef.current = parsed; setRouteData(parsed); setBestRouteIdx(0);
      }
    } catch (e) { console.warn('[fetchRoute] Directions API error:', e); }
    polylinesRef.current.forEach((r) => { r.out?.setMap(null); r.poly?.setMap(null); });
    polylinesRef.current = [];
    parsed.forEach((r, i) => {
      if (!r.fp?.length) return;
      const isBest = i === 0, opacity = isBest ? 1 : 0.45, weight = isBest ? 9 : 5, color = isBest ? '#1A73E8' : '#90CAF9', outW = isBest ? 15 : 9;
      ...
      if (!isBest) { poly.addListener('click', () => { const reordered = [r, ...parsed.filter((x) => x !== r)]; routeDataRef.current = reordered; setRouteData(reordered); setBestRouteIdx(0); fetchRoute(lat, lng, uid); }); }
    });
    if (parsed.length > 0) { const best = parsed[0]; const tr = trafficRatio(best.durationS, best.trafficS); const tl = trafficLabel(tr); addTLog(`${fmtDist(best.distanceM)} · ${fmtTime(best.trafficS)} · ${tl.label} (${parsed.length} routes)`, 'info', uid); }
    const v2 = await fetchRoutesV2(lat, lng, dest.latitude, dest.longitude);
    if (uid !== activeUnitIdRef.current) return;
    if (v2?.intervals?.length > 0) {
      const v2path = decPoly(v2.encoded); let overlayCount = 0;
      v2.intervals.forEach((seg, i) => {
        const color = speedToColor(seg.speed); if (!color) return;
        ...
        const segPath = v2path.slice(start, end + 1); if (segPath.length < 2) return;
        ...
        polylinesRef.current.push({ out: segOut, poly: segPoly, fp: segPath }); overlayCount++;
      });
    ...
    if (parsed[0]?.fp?.length) { const bounds = new window.google.maps.LatLngBounds(); parsed[0].fp.forEach((p) => bounds.extend(p)); if (destMkr.current) bounds.extend(destMkr.current.getPosition()); if (!bounds.isEmpty()) mapObj.current.fitBounds(bounds, { top: 80, bottom: 40, left: 20, right: 20 }); }
  }, [alertObj, addTLog]);
  ...
  const updateVehicle = useCallback((lat, lng, uid) => {
    ...
    if (!vehMkrMap.current[uid]) {
      const mkr = new window.google.maps.Marker({ position: pos, map: mapObj.current, zIndex: 100, icon: { url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44"><circle cx="22" cy="22" r="20" fill="${cfg.color}" stroke="white" stroke-width="2.5"/><text x="22" y="28" text-anchor="middle" font-size="16" fill="white" font-family="Arial" font-weight="bold">${alertObj.vehicleType[0].toUpperCase()}</text></svg>`), scaledSize: new window.google.maps.Size(44, 44), anchor: new window.google.maps.Point(22, 22) } });
      vehMkrMap.current[uid] = mkr;
      addTLog(`Unit on map [${uid}]`, 'ok', uid);
      setTimeout(() => { if (uid !== activeUnitIdRef.current) return; const b = new window.google.maps.LatLngBounds(); if (vehMkrMap.current[uid]) b.extend(vehMkrMap.current[uid].getPosition()); if (destMkr.current) b.extend(destMkr.current.getPosition()); if (!b.isEmpty()) mapObj.current.fitBounds(b); }, 600);
    } else { vehMkrMap.current[uid].setPosition(pos); }
    if (lastVRef.current) { const { lat: plat, lng: plng } = lastVRef.current; if (Math.abs(lat - plat) > 0.0001 || Math.abs(lng - plng) > 0.0001) mapObj.current.panTo(pos); }
    ...
    try {
      const searches = Object.entries(NEARBY_TYPES).map(([key, cfg]) => new Promise((resolve) => {
        ...
        svc.nearbySearch({ location: { lat, lng }, radius: cfg.radius, type: cfg.placeType }, (results, status) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK && results) resolve({ key, places: results.map((p) => ({ name: p.name, lat: p.geometry.location.lat(), lng: p.geometry.location.lng(), vicinity: p.vicinity || '', rating: p.rating || null, place_id: p.place_id })) });
      ...
      results.forEach((r) => {
        ...
        const { key, places } = r.value; counts[key] = places.length;
        ...
        places.forEach((p) => {
          ...
          const d = haversine(lat, lng, p.lat, p.lng); allPlaces.push({ ...p, d, cfg: NEARBY_TYPES[key] });
        });
      });
      setNearbyCounts((prev) => ({ ...prev, ...counts }));
      allPlaces.sort((a, b) => a.d - b.d); setNearbyList(allPlaces.slice(0, 20));
    } catch (_) {}
    nearbyInFlight.current = false;
  }, [nearbyLayers]);

  const toggleNearbyLayer = (key) => {
    setNearbyLayers((prev) => { const next = { ...prev, [key]: !prev[key] }; if (nearbyMkrsRef.current[key]) nearbyMkrsRef.current[key].forEach((m) => m.setMap(next[key] ? mapObj.current : null)); return next; });
  };
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (res.data && (res.data.eventPayload !== undefined || res.data.channel !== undefined)) {
          ...
          if (payloads && payloads.length > 0) { d = payloads[payloads.length - 1]; lastEventId = d.eventId || lastEventId || d.timestamp; }
          else { setTimeout(poll, 100); return; }
        } else { d = res.data; }

        if (!d || typeof d !== 'object') { setTimeout(poll, 100); return; }
        if (d.ticket_no && ticketNo && d.ticket_no !== ticketNo) { setTimeout(poll, 100); return; }
        ...
        if (isDowngradeToIdle) { setTimeout(poll, 100); return; }
        ...
        if (!isMounted || activeUnitIdRef.current !== myUid) return;
        const isTimeout = err?.code === 'ECONNABORTED' || err?.code === 'ERR_CANCELED' || err?.message === 'canceled' || /timeout/i.test(err?.message || '');
        if (err.response?.status === 408 || isTimeout) { setTimeout(poll, 100); }
  ...
  }, [activeUnitId, updateVehicle, fetchRoute, fetchNearby, alertObj, addTLog, tryCloseTicket]);
  ...
  useEffect(() => {
    const pollUnits = async () => {
      ...
      try {
        ...
        allLocs.forEach((u) => { if (units.includes(u.id)) { newLocs[u.id] = { latitude: u.latitude, longitude: u.longitude, speed: u.speed }; newTypes[u.id] = u.type || 'ambulance'; } });
        setUnitLocations((prev) => { const moved = units.some((uid) => { const p = prev[uid], n = newLocs[uid]; if (!p && !n) return false; if (!p || !n) return true; return p.latitude !== n.latitude || p.longitude !== n.longitude; }); return moved ? { ...prev, ...newLocs } : prev; });
        ...
        const nonActive = units.filter((uid) => uid !== activeUnitIdRef.current);
        if (!nonActive.length) return;

        const statusResults = await Promise.allSettled(
          nonActive.map((uid) =>
            axios.get(`${API_BASE_URL}/unit-location/${encodeURIComponent(uid)}`, {
        ...
        statusResults.forEach((r, i) => {
          ...
          if (TERMINAL_STATUSES.includes(unitStatusesRef.current[uid]) && ts !== 'completed') return;
          ...
          // Update React state
          setUnitStatuses((prev) => {
            if (TERMINAL_STATUSES.includes(prev[uid]) || prev[uid] === ts) return prev;
            return { ...prev, [uid]: ts };
          });
    ...
    const ensureDBClosed = async () => {
      ...
      try {
        ...
        if (!allUnitsCompletedInDB) {
          ...
          addTLog(`ensureDBClosed: waiting for ${pending.join(', ')} in DB`, 'info');
          return;
        }
        ...
        addTLog(`Stage 4 EXIT logged on mount → ticket ${ticketId} (${units.length} units)`, 'ok');
      } catch (err) {
        console.warn('[ensureDBClosed] failed:', err.message);
        addTLog(`ensureDBClosed failed: ${err?.response?.data?.error || err.message}`, 'warn');
      }
    };
  ...
  const handleSwitchUnit = useCallback((uid) => {
    if (uid === activeUnitIdRef.current) return;
    activeUnitIdRef.current = uid; lastTripStRef.current = '';
    setActiveUnitId(uid);
    ...
    setLive(false);
    setStats({ speed: 0, distM: 0, timeS: 0, lat: null, lng: null });
    setRouteData([]); setStepInfo({ idx: 0, total: 0 });
    setUnitLocations((prev) => { const loc = prev[uid]; if (loc?.latitude && mapObj.current) { mapObj.current.panTo({ lat: parseFloat(loc.latitude), lng: parseFloat(loc.longitude) }); mapObj.current.setZoom(15); } return prev; });
  ...
  return (
    <div style={s.root}>
      <div style={s.mapWrap}>
        <div style={s.topBar}>
          <button style={s.backBtn} onClick={() => navigate('/alerts')}>
            <ArrowLeftOutlined style={{ fontSize: '12px', verticalAlign: 'middle' }} /> Back
          </button>
          <button style={{ ...s.backBtn, background: 'rgba(124,58,237,.15)', borderColor: 'rgba(124,58,237,.4)', color: '#C4B5FD', fontWeight: 800 }} onClick={() => navigate(`/timeline/${id}`, { state: { alert: alertObj } })}>
            <HistoryOutlined style={{ fontSize: '12px', verticalAlign: 'middle' }} /> Timeline
          </button>
          <div style={s.alertPill}>
            <span style={{ fontSize: 19, display: 'flex', alignItems: 'center' }}>{cfg.icon}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 7 }}>
            ...
            <button style={{ ...s.backBtn, ...(trafficOn ? s.trafficOn : s.trafficOff) }} onClick={toggleTraffic}>
              <NodeIndexOutlined style={{ fontSize: '12px', verticalAlign: 'middle' }} /> Traffic: {trafficOn ? 'ON' : 'OFF'}
            </button>
            {ticketStatus === 'completed' && (
              <button style={{ ...s.backBtn, background: 'rgba(26,115,232,.15)', borderColor: 'rgba(26,115,232,.4)', color: '#82B4FF', fontWeight: 800 }} onClick={() => navigate(`/replay/${id}`, { state: { alert: alertObj } })}>
        ...
        <TicketDetailsOverlay alertObj={alertObj} agentTicket={agentTicket} ticketStatus={ticketStatus} dispatchedUnits={dispatchedUnits} unitStatuses={unitStatuses} unitTypes={unitTypes} activeUnitId={activeUnitId} onSwitchUnit={handleSwitchUnit} />

        <div style={s.nearbyLeg}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={s.nearbyLegTitle}><EnvironmentOutlined style={{ fontSize: '10px', verticalAlign: 'middle', marginRight: 4 }} /> Nearby Places</div>
            <button onClick={() => setNearbyOpen((v) => !v)} style={{ background: 'transparent', border: 'none', color: '#8B949E', cursor: 'pointer', fontSize: 12 }}>
              {nearbyOpen ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}
            </button>
          </div>
        ...
        <MiniMapOverlay alertObj={alertObj} dispatchedUnits={dispatchedUnits} unitLocations={unitLocations} unitTypes={unitTypes} onUnitClick={handleMiniMapUnitClick} activeUnitId={activeUnitId} />
        ...
        {!live && (
          <div style={s.noLocMsg}>
            ...
            <div style={{ fontSize: 11, opacity: 0.6, maxWidth: 240, textAlign: 'center' }}>Live position appears once the unit is moving</div>
          </div>
        )}
      </div>
      ...
      <div style={s.side}>
        <div style={s.section}>
          ...
          <div style={{ ...s.tsb, background: tsCfg.bg, borderColor: tsCfg.brd, color: tsCfg.c }}>
            <span style={{ fontSize: 24, flexShrink: 0, display: 'flex', alignItems: 'center' }}>{tsCfg.i}</span>
            <div>
              <div style={{ fontSize: 14, fontWeight: 800 }}>{tsCfg.t}</div>
              <div style={{ fontSize: 10, marginTop: 2, opacity: 0.7 }}>{tsCfg.s}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ ...s.section, ...s.infoGrid }}>
          {[
            { label: 'Status', val: live ? 'Online' : 'Offline', icon: <CheckCircleFilled style={{ color: live ? '#52c41a' : '#8B949E', fontSize: '10px' }} />, cls: live ? 'green' : '' },
            { label: 'Updated', val: new Date().toLocaleTimeString(), icon: <ClockCircleOutlined style={{ fontSize: '10px' }} />, cls: 'blue' },
            { label: 'Latitude', val: stats.lat?.toFixed(5) || '—', icon: <AimOutlined style={{ fontSize: '10px' }} />, cls: '' },
            { label: 'Longitude', val: stats.lng?.toFixed(5) || '—', icon: <AimOutlined style={{ fontSize: '10px' }} />, cls: '' },
            { label: 'Speed', val: stats.speed.toFixed(1) + ' km/h', icon: <RadarChartOutlined style={{ fontSize: '10px' }} />, cls: 'green', bar: Math.min(100, (stats.speed / 120) * 100) },
            { label: 'Dist', val: stats.distM > 0 ? fmtDist(stats.distM) : '—', icon: <NodeIndexOutlined style={{ fontSize: '10px' }} />, cls: 'blue' },
            { label: 'ETA', val: etaStr, icon: <ClockCircleOutlined style={{ fontSize: '10px' }} />, cls: 'yellow' },
            { label: 'Arrival', val: arrivalStr, icon: <ClockCircleOutlined style={{ fontSize: '10px' }} />, cls: '' },
          ].map((item) => (
            <div key={item.label} style={s.infoStat}>
              <div style={s.infoLabel}>{item.icon} {item.label}</div>
              <div style={{ ...s.infoVal, color: item.cls === 'green' ? '#34A853' : item.cls === 'blue' ? '#1A73E8' : item.cls === 'yellow' ? '#F9A825' : '#E6EDF3' }}>{item.val}</div>
              {item.bar !== undefined && (
                <div style={{ marginTop: 5, height: 3, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: 3, borderRadius: 2, background: stats.speed > 90 ? '#E53935' : stats.speed > 60 ? '#F9A825' : '#34A853', transition: 'width .8s', width: item.bar + '%' }} />
                </div>
              )}
            </div>
          ))}
        ...
        <div style={s.section}>
          ...
          {[
            { lbl: 'Unit Type', val: cfg.label, icon: <MedicineBoxOutlined style={{ fontSize: '10px' }} /> },
            { lbl: 'Severity', val: (alertObj?.severity || '').toUpperCase(), icon: <ExclamationCircleOutlined style={{ fontSize: '10px' }} /> },
            { lbl: 'Contact', val: alertObj?.phone || '—', icon: <PhoneOutlined style={{ fontSize: '10px' }} /> },
            { lbl: 'Destination', val: alertObj?.destination ? `${alertObj.destination.latitude?.toFixed(4)}, ${alertObj.destination.longitude?.toFixed(4)}` : '—', icon: <EnvironmentOutlined style={{ fontSize: '10px' }} /> },
            { lbl: 'Notes', val: alertObj?.notes || '—', icon: <FileTextOutlined style={{ fontSize: '10px' }} /> },
          ].map(({ lbl, val, icon }) => (
            <div key={lbl} style={s.aiRow}>
              <span style={s.aiLbl}>{icon} {lbl}</span>
        ...
        <div style={{ ...s.section, borderBottom: '1px solid #30363D' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={s.sectionLabel}><PushpinOutlined style={{ fontSize: '10px', verticalAlign: 'middle', marginRight: 4 }} /> Nearby Places</div>
            <button onClick={() => setNearbyPanelOpen((v) => !v)} style={{ background: 'transparent', border: 'none', color: '#8B949E', cursor: 'pointer' }}>
              {nearbyPanelOpen ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}
            </button>
          </div>
          {nearbyPanelOpen && (nearbyList.length === 0 ? (
            ...
            nearbyList.map((p, i) => (
              <div key={i} style={s.npItem}>
                <span style={{ fontSize: 16, display: 'flex', alignItems: 'center' }}>{p.cfg.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={s.npName}>{p.name}</div>
                  <div style={{ display: 'flex', gap: 8, fontSize: 10, color: '#8B949E' }}><span>📏 {fmtDist(p.d)}</span><span>{p.cfg.label}</span></div>
                </div>
              </div>
            ))
          ))}
          <button style={s.refreshBtn} onClick={() => { nearbySearchCenter.current = null; if (lastVRef.current) fetchNearby(lastVRef.current.lat, lastVRef.current.lng); }}>
            <ReloadOutlined style={{ fontSize: '11px', verticalAlign: 'middle', marginRight: 4 }} /> Refresh Nearby
          </button>
        </div>
...
const s = {
  ...
  nearbyLeg: { position: 'absolute', bottom: 20, left: 85, zIndex: 20, background: 'rgba(13,17,23,.92)', border: '1px solid #30363D', borderRadius: 12, padding: '10px 14px', backdropFilter: 'blur(8px)', minWidth: 200 },
