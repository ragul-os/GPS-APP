...
const UCFG = {
  ambulance: { icon: '🚑', label: 'Ambulance',  color: '#4CAF50' },
  fire:      { icon: '🚒', label: 'Fire Engine', color: '#FB8C00' },
  police:    { icon: '🚔', label: 'Police Unit', color: '#1E88E5' },
  rescue:    { icon: '🚁', label: 'Rescue',      color: '#8E24AA' },
...
const TICKET_STATUS_CFG = {
  ...
  rejected:   { label: '❌ Rejected',   color: '#E53935', bg: 'rgba(229,57,53,.15)'   },
};
...
const UNIT_ST_ICON = {
  idle:'⏸', accepted:'✅', en_route:'🚑', on_action:'🔵',
  arrived:'🏁', completed:'✅', abandoned:'🚫',
};

const TRIP_STATUS_CFG = {
  idle:       { bg:'rgba(139,148,158,.12)', brd:'rgba(139,148,158,.2)',  c:'#8B949E', i:'⏸',  t:'Idle',             s:'No active trip' },
  dispatched: { bg:'rgba(249,168,37,.15)',  brd:'rgba(249,168,37,.3)',   c:'#F9A825', i:'🚨',  t:'Alert Dispatched', s:'Unit notified — preparing' },
  en_route:   { bg:'rgba(26,115,232,.15)',  brd:'rgba(26,115,232,.3)',   c:'#1A73E8', i:'🚑',  t:'En Route',         s:'Unit driving to location' },
  on_action:  { bg:'rgba(156,39,176,.15)',  brd:'rgba(206,147,216,.3)', c:'#CE93D8', i:'🔵',  t:'On Action',        s:'Unit on scene' },
  arrived:    { bg:'rgba(52,168,83,.18)',   brd:'rgba(52,168,83,.4)',    c:'#34A853', i:'🏁',  t:'ARRIVED',          s:'Unit reached destination!' },
  completed:  { bg:'rgba(52,168,83,.10)',   brd:'rgba(52,168,83,.25)',   c:'#A5D6A7', i:'✅',  t:'Trip Completed',   s:'Successfully completed' },
  abandoned:  { bg:'rgba(183,28,28,.15)',   brd:'rgba(229,57,53,.35)',   c:'#EF5350', i:'🚫',  t:'Trip Abandoned',   s:'Driver abandoned trip' },
};
...
function trafficLabel(r){if(r<1.1)return{label:'Free flow',color:'#34A853',emoji:'🟢',short:'Free'};if(r<1.3)return{label:'Moderate',color:'#F9A825',emoji:'🟡',short:'Mod'};if(r<1.6)return{label:'Heavy',color:'#E53935',emoji:'🔴',short:'Heavy'};return{label:'Stop & Go',color:'#B71C1C',emoji:'🔴',short:'Stop&Go'};}
...
export default function LiveTrackingPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        const now = Date.now();
        if (alertObj?.destination && now-routeAtRef.current>8000) { routeAtRef.current=now; fetchRoute(lat,lng,myUid); }
        fetchNearby(lat, lng);
  ...
  useEffect(() => {
    ...
    pollUnits();
    const iv = setInterval(pollUnits, 1000);
    return () => clearInterval(iv);
  }, [addTLog]);
