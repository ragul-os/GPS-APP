...
function trafficLabel(r) { if (r < 1.1) return { label: 'Free flow', color: '#34A853', icon: <CheckCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#52c41a' }} />, short: 'Free' }; if (r < 1.3) return { label: 'Moderate', color: '#F9A825', icon: <ClockCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#faad14' }} />, short: 'Mod' }; if (r < 1.6) return { label: 'Heavy', color: '#E53935', icon: <ExclamationCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#ff4d4f' }} />, short: 'Heavy' }; return { label: 'Stop & Go', color: '#B71C1C', icon: <ExclamationCircleFilled style={{ fontSize: '12px', verticalAlign: 'middle', color: '#ff4d4f' }} />, short: 'Stop&Go' }; }
  ...
  return (
    <div style={ns.ticketOverlay}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginBottom: collapsed ? 0 : 8 }}>
        ...
        <button onClick={() => setCollapsed(v => !v)} style={ns.collapseBtn}>{collapsed ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}</button>
      </div>
                    ...
                    {dispatchedUnits.map(uid => {
                      ...
                      return (
                        <button key={uid} onClick={() => onSwitchUnit && onSwitchUnit(uid)}
                          style={{ ...ns.switcherBtn, background: isActive ? `${ucfg.color}22` : 'rgba(30,37,46,0.7)', border: `1.5px solid ${isActive ? ucfg.color : 'rgba(48,54,61,0.7)'}`, color: isActive ? ucfg.color : '#8B949E', boxShadow: isActive ? `0 0 10px ${ucfg.color}44` : 'none' }}>
              ...
              {dispatchedUnits.length === 1 && (
                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 7, padding: '6px 9px', borderRadius: 8, background: 'rgba(26,115,232,0.08)', border: '1px solid rgba(26,115,232,0.2)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 7, color: '#82B4FF', fontWeight: 700, marginTop: 1, display: 'flex', alignItems: 'center', gap: 3 }}><CheckCircleFilled style={{ color: '#ff4d4f', fontSize: '6px' }} /> LIVE TRACKING</div>
                  </div>
                </div>
              )}
            </div>
          ...
          : filteredLogs.map((l, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 2, lineHeight: 1.5 }}>
                ...
                <span style={{ color: logColors[l.type] || '#8B949E', flex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
...
export default function LiveTrackingPage() {
  ...
  return (
    <div style={s.root}>
      <div style={s.mapWrap}>
        <div style={s.topBar}>
          <button style={s.backBtn} onClick={() => navigate('/alerts')}><ArrowLeftOutlined style={{ fontSize: '12px', verticalAlign: 'middle' }} /> Back</button>
        ...
        <div style={s.nearbyLeg}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setNearbyOpen(v => !v)} style={{ background: 'transparent', border: 'none', color: '#8B949E', cursor: 'pointer', fontSize: 12 }}>{nearbyOpen ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}</button>
          </div>
      ...
      <div style={s.side}>
        ...
        <div style={{ ...s.section, ...s.infoGrid }}>
          {[
            ...
            <div key={item.label} style={s.infoStat}>
              ...
              {item.bar !== undefined && <div style={{ marginTop: 5, height: 3, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' }}><div style={{ height: 3, borderRadius: 2, background: stats.speed > 90 ? '#E53935' : stats.speed > 60 ? '#F9A825' : '#34A853', transition: 'width .8s', width: item.bar + '%' }} /></div>}
            </div>
          ))}
        </div>
        ...
        <div style={s.section}>
          ...
          {[
            ...
            <div key={lbl} style={s.aiRow}><span style={s.aiLbl}>{icon} {lbl}</span><span style={s.aiVal}>{val}</span></div>
          ))}
        </div>
        ...
        <div style={{ ...s.section, borderBottom: '1px solid #30363D' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setNearbyPanelOpen(v => !v)} style={{ background: 'transparent', border: 'none', color: '#8B949E', cursor: 'pointer' }}>{nearbyPanelOpen ? <DownOutlined style={{ fontSize: '9px' }} /> : <UpOutlined style={{ fontSize: '9px' }} />}</button>
          </div>
          {nearbyPanelOpen && (nearbyList.length === 0
            ...
            : nearbyList.map((p, i) => (<div key={i} style={s.npItem}><span style={{ fontSize: 16, display: 'flex', alignItems: 'center' }}>{p.cfg.icon}</span><div style={{ flex: 1 }}><div style={s.npName}>{p.name}</div><div style={{ display: 'flex', gap: 8, fontSize: 10, color: '#8B949E' }}><span>📏 {fmtDist(p.d)}</span><span>{p.cfg.label}</span></div></div></div>))
          )}
          <button style={s.refreshBtn} onClick={() => { nearbySearchCenter.current = null; if (lastVRef.current) fetchNearby(lastVRef.current.lat, lastVRef.current.lng); }}><ReloadOutlined style={{ fontSize: '11px', verticalAlign: 'middle', marginRight: 4 }} /> Refresh Nearby</button>
        </div>
...
const s = {
  ...
  chatBubble: { position: 'absolute', bottom: 20, left: 16, zIndex: 60, pointerEvents: 'auto' },
};
