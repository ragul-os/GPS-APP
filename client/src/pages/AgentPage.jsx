...
const FALLBACK_FORMS = {
  ...
  fire: [
    ...
    { id:'f5', type:'segmented',label:'Priority',        required:true, options:['Low','Medium','High','Critical'] },
    { id:'f6', type:'radio',    label:'People Trapped?', required:false, options:['Unknown','None','Yes — people trapped'] },
  ...
  police: [
    ...
    { id:'f5', type:'segmented',label:'Priority',       required:true, options:['Low','Medium','High','Critical'] },
    { id:'f6', type:'radio',    label:'Armed?',         required:false, options:['Unknown','No','Yes — armed'] },
    { id:'f7', type:'textarea', label:'Notes',          required:false },
  ],
  rescue: [
...
function FormField({ field, value, onChange }) {
  ...
  switch (field.type) {
    ...
    case 'dropdown':
      return (
        <select value={value || ''} onChange={e => onChange(field.id, e.target.value)} style={base}>
    ...
    case 'segmented':
      return (
        <div style={{ display: 'flex', gap: 6 }}>
          {(field.options || []).map(o => (
            <button key={o} type="button" onClick={() => handleSegment(o)}
              style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {['Yes', 'No'].map(o => (
            <label key={o} style={{
              ...
              border: '1px solid',
              borderColor: value === o ? (o === 'Yes' ? '#34A853' : '#E53935') : '#30363D',
              background:  value === o ? (o === 'Yes' ? 'rgba(52,168,83,.1)' : 'rgba(229,57,53,.1)') : '#0D1117',
    ...
    default:
      return (
        <input type={field.type === 'phone' ? 'tel' : field.type === 'number' ? 'number' : 'text'}
...
export default function AgentPage() {
  ...
  const handleSubmit = () => {
    setError('');
    if (!pickedLat || !pickedLng) { setError('Please pick a location on the map first.'); return; }
  ...
  const handleReset = () => {
    setSubmitted(false); setLastTicket(null);
    setAnswers({}); setPickedLat(null); setPickedLng(null);
    setUnitType('ambulance'); setSeverity('critical'); setError('');
  };
        ...
        <div style={{ background: '#161B22', border: '1px solid rgba(52,168,83,.4)', borderRadius: 20, padding: 36, maxWidth: 500, width: '100%', textAlign: 'center' }}>
          ...
          <div style={{ background: '#0D1117', border: '1px solid #30363D', borderRadius: 12, padding: '14px 18px', marginBottom: 24, textAlign: 'left' }}>
            ...
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(48,54,61,.5)', fontSize: 12 }}>
                <span style={{ color: '#8B949E', fontWeight: 600 }}>{label}</span>
                <span style={{ fontWeight: 700, color: '#E6EDF3', maxWidth: 260, textAlign: 'right' }}>{val}</span>
              </div>
            ))}
          </div>
      ...
      <div style={s.card}>
        ...
        {fields.map(field => (
          <div key={field.id} style={{ marginBottom: 11 }}>
            ...
            <FormField field={field} value={answers[field.id]} onChange={handleAnswer} />
          </div>
        ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
          ...
          <div>
            ...
            <input readOnly value={pickedLng?.toFixed(6) || ''} placeholder="76.9558" style={{ ...s.input, opacity: .7 }} />
          </div>
        </div>
...
const s = {
  ...
  dispatchBtn: {
    width: '100%', padding: '12px 18px', borderRadius: 11, border: 'none',
    color: '#fff', fontFamily: 'Sora, sans-serif', fontSize: 14, fontWeight: 800,
