...
function updateAgentTicket(ticketId, patch) {
  ...
  if (idx !== -1) { tickets[idx] = { ...tickets[idx], ...patch }; saveAgentTickets(tickets); }
}
...
/* ── Confirm Modal ── */
function ConfirmModal({ open, ticket, severity, answers, pickedLat, pickedLng, selectedUnitIds, allUnits, onCancel, onConfirm, loading }) {
  if (!open) return null;
        ...
        <div style={s.mHead}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#8B949E', marginTop: 2 }}>
              {selectedUnitIds.length > 0 ? `${selectedUnitIds.length} unit(s) will be dispatched` : 'Will broadcast to ALL available units'}
            </div>
          </div>
        </div>
    ...
    <div style={s.ticketBox}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 9px', borderRadius: 7, background: `${SEV_COLORS[ticket.severity]}18`, color: SEV_COLORS[ticket.severity], textTransform: 'uppercase' }}>{ticket.severity}</span>
          <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 9px', borderRadius: 7, background: stCfg.bg, color: stCfg.color }}>{stCfg.label}</span>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{ width: 40, height: 40, borderRadius: 11, flexShrink: 0, background: `${cfg.color}18`, border: `1.5px solid ${cfg.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>{cfg.icon}</div>
        <div><div style={{ fontSize: 15, fontWeight: 800 }}>{ticket.name}</div><div style={{ fontSize: 10, color: cfg.color, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>{cfg.label}</div></div>
      </div>
      {rows.map(([label, val], i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '5px 0', borderBottom: '1px solid rgba(48,54,61,.5)', gap: 8 }}>
          <span style={{ fontSize: 10, color: '#8B949E', fontWeight: 600, flexShrink: 0 }}>{label}</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: label === 'Coordinates' ? '#1A73E8' : '#E6EDF3', textAlign: 'right', wordBreak: 'break-word', maxWidth: 200, fontFamily: label === 'Coordinates' ? 'JetBrains Mono, monospace' : 'inherit' }}>{val}</span>
        </div>
      ))}
      {dispatchedUnits?.length > 0 && (
        <div style={{ marginTop: 10 }}>
          <div style={{ fontSize: 9, color: '#8B949E', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 5 }}>Dispatched Units ({dispatchedUnits.length})</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {dispatchedUnits.map((uid, i) => <span key={i} style={{ fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 7, background: 'rgba(26,115,232,.12)', color: '#82B4FF', border: '1px solid rgba(26,115,232,.2)' }}>{uid}</span>)}
          </div>
        </div>
      )}
      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, color: '#30363D', marginTop: 10 }}>{ticket.id}</div>
    </div>
  );
}
...
function TicketSwitcherStrip({ tickets, activeTicketId, onSelect }) {
  ...
  return (
    <div style={s.stripWrap}>
      <div style={{ fontSize: 9, color: '#8B949E', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, flexShrink: 0, marginRight: 10 }}>Tickets ({tickets.length}):</div>
      <div style={s.stripScroll}>
        {tickets.map(ticket => {
          ...
          return (
            <button key={ticket.id} onClick={() => onSelect(ticket)} title={ticket.name + ' — ' + ticket.address}
              style={{ ...s.stripPill, ...(isAct ? { borderColor: cfg.color, background: `${cfg.color}18`, color: cfg.color } : {}) }}>
              <span style={{ fontSize: 13 }}>{cfg.icon}</span>
              <span style={{ fontWeight: 800, fontSize: 11, maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ticket.name}</span>
              <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 5, background: stCfg.bg, color: stCfg.color, flexShrink: 0 }}>{ticket.status}</span>
            </button>
          );
        })}
      </div>
          ...
          {filteredTickets.map(ticket => {
            ...
            return (
              <div key={ticket.id} style={{ ...s.ticket, ...(isPending ? { border: '1px solid rgba(249,168,37,.4)', boxShadow: '0 0 0 2px rgba(249,168,37,.08)' } : {}) }} onClick={() => onSelectTicket(ticket)}>
                ...
                <div style={{ display: 'inline-block', fontSize: 9, fontWeight: 800, padding: '2px 8px', borderRadius: 5, letterSpacing: .5, marginBottom: 8, background: stCfg.bg, color: stCfg.color }}>{stCfg.label}</div>
                ...
                <div style={s.ticketInner}>
                  ...
                  <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, color: '#30363D', marginTop: 8 }}>{ticket.id}</div>
                </div>
              </div>
            );
          })}
...
function InlineUnitSelector({ units, selectedUnitIds, onToggleUnit }) {
  ...
  return (
    <div style={s.inlineUnitWrap}>
      ...
      <div style={s.inlineUnitHeader}>
        ...
        {selectedUnitIds.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 10, fontWeight: 800, color: '#82B4FF' }}>{selectedUnitIds.length} selected</span>
            <button
              style={s.clearSelBtn}
      ...
      <div style={{ fontSize: 10, color: '#8B949E', marginBottom: 10, lineHeight: 1.5 }}>
        Click to select · multiple units dispatched together · leave none to broadcast all
      </div>
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  {isSel && <span style={{ fontSize: 9, color: '#fff', fontWeight: 900, lineHeight: 1 }}>✓</span>}
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: '#E6EDF3', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.name}</span>
                    {u._isMock && <span style={{ fontSize: 7, background: 'rgba(249,168,37,.15)', color: '#F9A825', padding: '1px 4px', borderRadius: 3, flexShrink: 0 }}>MOCK</span>}
                  </div>
                  <div style={{ fontSize: 9, color: '#8B949E', marginTop: 1 }}>
                    {u.id}
                    {u.distanceM != null && ` · 📍 ${u.distanceM >= 1000 ? (u.distanceM / 1000).toFixed(1) + ' km' : u.distanceM + ' m'}`}
                  </div>
                </div>

                <span style={{
                  fontSize: 9, fontWeight: 800, padding: '2px 7px', borderRadius: 6, flexShrink: 0,
...
export default function DispatchPage() {
  ...
  function loadTicket(t) {
    setSelectedTicket(t); setAgentTicket(t);
    ...
    <div>
      ...
      <div style={s.outerLayout}>
        ...
        <div>
          ...
          <div style={{ marginBottom: 14 }}>
            <button
              style={{ ...s.toggleResBtn, ...(pickedLat ? s.toggleResBtnEnabled : {}), ...(showResources ? s.toggleResBtnActive : {}) }}
              disabled={!pickedLat}
            ...
            <div style={s.card}>
              ...
              {selectedUnitIds.length > 0 && (
                <div style={s.selUnitsSummary}>
                  <div style={{ fontSize: 9, color: '#82B4FF', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 7 }}>
                    {selectedUnitIds.length} Unit(s) Selected
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {selectedUnitIds.map(uid => {
                      const unit = unitList.find(u => u.id === uid);
                      const ucfg = unit ? (UCFG[unit.type] || UCFG.ambulance) : UCFG.ambulance;
                      return (
        ...
        <div>
          ...
          <div style={s.card}>
            ...
            {lastAlertIds.length > 0 && (
              <>
                ...
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: '#1A73E8', marginBottom: 11, lineHeight: 1.8 }}>
                  {lastAlertIds.map((id, i) => <div key={i}>{id}</div>)}
                </div>
              </>
            )}
          ...
          {/* Ticket details */}
          <TicketDetailsBox ticket={agentTicket} dispatchedUnits={dispatchedUnits} />
