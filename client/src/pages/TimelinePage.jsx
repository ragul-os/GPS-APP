...
const EVENT_CFG = {
  ...
  ASSIGNED_UNITS: {
    ...
    icon: 'M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0zM3 9h14M3 5h18v4H3z',
  },
...
// ── Event → intrinsic stage ──────────────────────────────────────────────────
// Mirrors server/ticketEvents.js deriveStage() so historical rows whose
// ticket_status was written under the old race-prone logic still group into
// the right stage section purely from the event name.
const EVENT_STAGE = {
  CREATED: 'Stage 1',
  UPDATED_INFO: 'Stage 1',
  ASSIGNED_DISPATCHER: 'Stage 2',
  ASSIGNED_UNITS: 'Stage 2',
  DISPATCHED: 'Stage 2',
  ACKNOWLEDGED: 'Stage 3',
  REJECTED: 'Stage 3',
  ENROUTE: 'Stage 3',
  EN_ROUTE: 'Stage 3',
  ARRIVED: 'Stage 3',
...
function eventStage(ev) {
  const key = (ev?.event || '').toUpperCase().replace(/\s+/g, '_');
  return EVENT_STAGE[key] || ev?.ticket_status || 'Unknown';
}
...
function unitColor(uid, map) {
  if (!map[uid])
    map[uid] = UNIT_PALETTE[Object.keys(map).length % UNIT_PALETTE.length];
  return map[uid];
}
...
function EventCard({ ev, side, palette, colorMap, index }) {
  ...
  const cfg = EVENT_CFG[eventKey] || {
    label: ev.event || 'Event',
    icon: EVENT_CFG.CREATED.icon,
  };
  const srcColor =
    SOURCE_COLORS[(ev.event_source || '').toLowerCase()] || '#64748B';
      ...
      {hasUnits && (
        ...
        >
          ...
          {Object.values(unitsObj).filter(Boolean).length > 3 && (
            <span
              style={{ fontSize: 8, color: '#4A5568', alignSelf: 'center' }}
            >
              +{Object.values(unitsObj).filter(Boolean).length - 3}
            </span>
          )}
      ...
      <div style={{ position: 'absolute', bottom: 5, right: 8 }}>
        <span
          style={{
            ...
            padding: '1px 5px',
            borderRadius: 2,
            background: `${palette.a}12`,
            color: `${palette.light}99`,
            border: `1px solid ${palette.a}25`,
            textTransform: 'uppercase',
            letterSpacing: 0.4,
          }}
        >
...
// ── StageSection ──────────────────────────────────────────────────────────────
// paletteIdx is 1-based stage number (1..4); fall back to section index for
// unknown stage keys so colors stay stable per stage rather than per render order.
function StageSection({ stageKey, label, events, paletteIdx, colorMap }) {
  const idx =
    Number.isFinite(paletteIdx) && paletteIdx > 0 ? paletteIdx - 1 : 0;
  const palette = PALETTES[idx % PALETTES.length];
  const spineTop = CARD_H / 2;
  const spineHeight =
    events.length > 1 ? (events.length - 1) * (CARD_H + ROW_GAP) : 0;
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
        <div
          style={{
            flex: 1,
            height: 1,
            background: `linear-gradient(90deg,transparent,${palette.a}40)`,
          }}
        />
        <div
          style={{
        ...
        <div
          style={{
            flex: 1,
            height: 1,
            background: `linear-gradient(90deg,${palette.a}40,transparent)`,
          }}
        />
      </div>
...
export default function TimelinePage() {
  ...
  // Group events by their intrinsic stage (derived from the event name)
  // rather than the stored ticket_status, so rows tagged under the old
  // high-water-mark deriveStage still render in the correct bucket.
  const grouped = useMemo(() => {
    const map = new Map();
    events.forEach((ev) => {
      const key = eventStage(ev);
  ...
  const unitSet = new Set(
    events
      .filter(
        (e) => (e.event_source || '').toLowerCase() === 'unit' && e.source_id,
      )
      .map((e) => e.source_id),
  );
    ...
    <div style={S.root}>
      ...
      <div style={S.topBar}>
        ...
        >
          ...
          {lastRefresh && (
            <span
              style={{ fontSize: 8, color: '#4A5568', fontFamily: 'monospace' }}
            >
      ...
      <div style={S.body}>
        ...
        {!loading &&
          !error &&
          grouped.map(([stageKey, evs], si) => {
            const sord = stageOrder(stageKey);
            const pidx = sord >= 1 && sord <= 4 ? sord : si + 1;
            return (
              <StageSection
                key={stageKey}
                stageKey={stageKey}
