...
export default function MapView({ onLocationPick, pickedLat, pickedLng, onFindNearest }) {
  ...
  useEffect(() => {
    ...
    if (window.google?.maps) init();
      ...
      if (!existing) {
        ...
        script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyAVz9omOzS8B29CEi50AkrNPMPn3JXhbcg&libraries=places&callback=__initMapView`;
        script.async = true;
      ...
      icon: {
        url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(
          `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="52" viewBox="0 0 40 52">
            <path d="M20 0C9 0 0 9 0 20C0 35 20 52 20 52C20 52 40 35 40 20C40 9 31 0 20 0Z"
              fill="${INCIDENT_COLOR}" stroke="white" stroke-width="2.5"/>
            <circle cx="20" cy="20" r="8" fill="white" opacity="0.9"/>
            <circle cx="20" cy="20" r="4" fill="${INCIDENT_COLOR}"/>
            <text x="20" y="15" text-anchor="middle" font-size="9" fill="white" font-weight="bold">DEST</text>
          </svg>`),
  ...
  const handlePreset = (p) => {
    ...
    setSearchVal(p.name);
    setResolvedAddr(p.name);
  };

  return (
    <div style={s.card}>
      <div style={s.cardTitle}>📍 Set Incident Location</div>

     
      <div style={s.presetRow}>
        {presets.map(p => (
          <button key={p.name} style={s.presetBtn} onClick={() => handlePreset(p)}>
            {p.name}
          </button>
        ))}
      </div>
      ...
      <div style={s.searchWrap}>
        ...
        />
        {searchVal && (
          <button style={s.clearBtn} onClick={clearSearch}>✕</button>
        )}
      </div>

      {resolvedAddr && (
        <div style={s.resolvedAddr}>✅ {resolvedAddr}</div>
      )}
      ...
      <div style={s.statusBar}>
        <div style={s.statusLeft}>
          <span style={{ ...s.dot, background: pickedLat ? '#34A853' : '#30363D' }} />
          <span style={s.statusInfo}>
            {pickedLat ? `Dest: ${pickedLat.toFixed(4)}, ${pickedLng.toFixed(4)}` : 'Click map to set destination'}
          </span>
        </div>
        <button style={s.findBtn} onClick={() => onFindNearest?.()}>🔍 Find Nearest Unit</button>
      </div>

      {pickedLat && (
        <div style={s.coordDisp}>
...
const s = {
  ...
  coordDisp: {
    ...
    padding: '7px 11px', background: 'rgba(26,115,232,.08)', borderRadius: 7,
  },
}; */
  ...
  return (
    <div style={s.card}>
      <div style={s.cardTitle}>
        📍 {locationLocked ? 'Incident Location (Agent Set)' : 'Set Incident Location'}
      </div>
