...
function shouldListAgentTicket(ticket, primaryAgentTicketId) {
  ...
  if (['dispatched', 'en_route', 'on_action'].includes(ticket.status))
    return true;
  if (ticket.status === 'pending' && (ticket.alertIds || []).length > 0)
    return true;
  return false;
}
...
export default function GlobalChatPanel({
  ...
  const handleOpenMembers = async (e, room) => {
    ...
    const listRect = e.currentTarget
      .closest('.room-list-scroll')
      ...
      const members = (res.chunk || [])
        ...
        .map((event) => ({
          ...
          displayName:
            ...
            event.state_key.replace(/^@/, '').split(':')[0],
        }));
  ...
  const loadRooms = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${SYNAPSE_BASE}/_matrix/client/v3/joined_rooms`,
        {
      ...
      if (res.ok) {
        ...
        console.log(
          '[GlobalChatPanel] joined_rooms found:',
          joined_rooms.length,
        );
        ...
        console.warn(
          '[GlobalChatPanel] joined_rooms request failed:',
          res.status,
        );
      }
      ...
      finalRooms.sort((a, b) => {
        ...
        const aActive = ['dispatched', 'en_route', 'on_action'].includes(
          a.alertObj?.status,
        );
        const bActive = ['dispatched', 'en_route', 'on_action'].includes(
          b.alertObj?.status,
        );
      ...
      if (!selectedRef.current && finalRooms.length > 0) {
        const initial =
          finalRooms.find((r) => r.ticketId === primaryTicketId) ||
          finalRooms[0];
  ...
  useEffect(() => {
    ...
    const selValid =
      selectedRoomId && rooms.some((r) => r.roomId === selectedRoomId);
      ...
      while (!ctrl.signal.aborted) {
        try {
          ...
          for (const [roomId, roomData] of Object.entries(joinedRooms)) {
            ...
            if (
              incoming.length > 0 &&
              !(open && selectedRef.current === roomId)
            ) {
  ...
  const onPanelDragStart = useCallback(
    (e) => {
      ...
      const onMove = (ev) => {
        ...
        setPanelPos({
          x: ev.clientX - dragPanelStart.current.x,
          y: ev.clientY - dragPanelStart.current.y,
        });
      };
      const onUp = () => {
  ...
  const onPanelResizeStart = useCallback(
    (e) => {
      ...
      const onMove = (ev) => {
        ...
        setPanelSize({
          w: Math.max(600, resizePanelStart.current.w + dw),
          h: Math.max(400, resizePanelStart.current.h + dh),
        });
      };
      const onUp = () => {
  ...
  const onDragStart = useCallback(
    (e) => {
      ...
      const onMove = (ev) => {
        ...
        setSidebarWidth(
          Math.max(180, Math.min(440, dragStartW.current + delta)),
        );
      };
      const onUp = () => {
    ...
    >
      ...
      >
        {dmLoading && (
          ...
          >
            <LoadingOutlined
              style={{ fontSize: 24, color: '#1A73E8' }}
              spin
            />
            <div style={{ fontSize: 11, color: '#82B4FF', fontWeight: 700 }}>
              Initiating Direct Message...
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: '#8B949E', marginTop: 1 }}>
                ...
                {totalUnread > 0 && (
                  <span
                    style={{ marginLeft: 8, color: '#1A73E8', fontWeight: 700 }}
                  >
                    · {totalUnread} unread
                  </span>
                )}
          ...
          <div style={{ display: 'flex', gap: 6, position: 'relative' }}>
            <button
              ...
              style={{
                ...
                height: 32,
                borderRadius: 8,
                background: isMembersOpen
                  ? 'rgba(26,115,232,.15)'
                  : 'rgba(139,148,158,.1)',
                border: '1px solid #30363D',
                color: isMembersOpen ? '#1A73E8' : '#8B949E',
                fontSize: 16,
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(26,115,232,.2)';
                e.currentTarget.style.color = '#1A73E8';
              }}
              onMouseLeave={(e) => {
                if (!isMembersOpen) {
                  e.currentTarget.style.background = 'rgba(139,148,158,.1)';
                          ...
                          <div style={{ position: 'relative', flexShrink: 0 }}>
                            ...
                            <div
                              style={{
                                ...
                                height: 8,
                                borderRadius: '50%',
                                background: m.online ? '#34A853' : '#8B949E',
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(229,57,53,.15)';
        ...
        >
          ...
          <div
            style={{
              ...
              flexShrink: 0,
              background: '#161B22',
              borderRight: sidebarCollapsed ? 'none' : '1px solid #30363D',
              display: 'flex',
              flexDirection: 'column',
              overflow: 'hidden',
          ...
          >
            ...
            >
              <div
                style={{
                  filter: showMembersForRoom ? 'blur(4px)' : 'none',
              ...
              >
                {filteredRooms.map((room) => {
                  ...
                  const alert =
                    isPrimary && primaryAlert
                      ? room.alertObj || primaryAlert
                      : room.alertObj;
                  const stCfg =
                    STATUS_CFG[alert?.status] || STATUS_CFG.dispatched;
                        ...
                        style={{
                          ...
                          background: isPrimary
                            ...
                            : isSelected
                              ? 'rgba(26,115,232,.1)'
                              : 'transparent',
                          borderLeft: `4px solid ${isPrimary ? '#34A853' : isSelected ? '#1A73E8' : 'transparent'}`,
                          boxShadow: isPrimary
                            ? 'inset 0 0 0 1px rgba(52,168,83,.35)'
                            : 'none',
                        }}
                      >
                        {isPrimary && (
                        ...
                        {!room.virtual && !room.isDirect && (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: 6,
                              background:
                                showMembersForRoom?.roomId === room.roomId
                                  ? '#1A73E8'
                                  : '#161B22',
                              border: '1px solid #30363D',
                              display: 'flex',
                ...
                <>
                  ...
                  <div
                    style={{
                      ...
                      right: 12,
                      top:
                        showMembersForRoom.position.top +
                        showMembersForRoom.position.height / 2,
                      transform: 'translateY(-50%)',
                      width: 28,
                      height: 28,
                      borderRadius: 6,
                  ...
                  <div
                    style={{
                      ...
                      top: Math.min(
                        showMembersForRoom.position.top +
                          showMembersForRoom.position.height,
                        400,
                      ),
                      left: 12,
                      right: 12,
                      ...
                      overflowY: 'auto',
                      animation:
                        'dropdownSlide 0.25s cubic-bezier(0.2, 0.8, 0.2, 1)',
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      ...
                      <span
                        onClick={() => setShowMembersForRoom(null)}
                        style={{ cursor: 'pointer', padding: 4, fontSize: 14 }}
                      >
                        ✕
                      </span>
                    </div>
                      ...
                      {showMembersForRoom.members.map((m) => (
                        <div
                          ...
                          style={{
                            padding: '10px 14px',
                            cursor:
                              m.userId === myUserId ? 'default' : 'pointer',
                            fontSize: 11,
                            color: '#E6EDF3',
                            display: 'flex',
                            alignItems: 'center',
                        ...
                        >
                          <div
                            style={{
                              ...
                              borderRadius: 15,
                              background: '#30363D',
                              border: `1px solid ${m.userId === myUserId ? '#34A853' : '#1A73E8'}`,
                              display: 'flex',
                              alignItems: 'center',
                              justifyContent: 'center',
                              fontSize: 10,
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div
                              style={{
                                ...
                                whiteSpace: 'nowrap',
                                color:
                                  m.userId === myUserId ? '#34A853' : '#E6EDF3',
                              }}
                            >
          ...
          >
            {!selectedRoom ? (
              ...
              <InteractionsTab
                ...
                initialRoomId={
                  selectedRoom.roomId.startsWith('virtual:')
                    ? null
                    : selectedRoom.roomId
                }
              />
        ...
        <div
          ...
          style={{
            ...
            pointerEvents: 'auto',
            background:
              'linear-gradient(135deg, transparent 50%, rgba(48,54,61,.5) 50%)',
          }}
        />
      </div>
