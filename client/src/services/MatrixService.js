...
// NOTE: `inviteUserIds` is intentionally ignored. The room is `public_chat`
// (join_rule=public), so any user can join with the room_id alone — the mobile
// units call joinRoom() on accept. Sending `invite: [...]` here forces Synapse
// to write an m.room.member state event for every invitee inside the
// createRoom transaction, which dominates the call latency. The parameter is
// kept for backwards compatibility with existing callers.
export async function createRoom(accessToken, roomName, _inviteUserIds = []) {
...
// ── 5a. Force-join a local user to a room via Synapse admin API ──────────────
// Bypasses the invite/join handshake so the user appears as `join` membership
// immediately. Required after createRoom (which no longer sends invites) so
// dispatched units land in the ticket room without needing the mobile app to
// be online or react to NATS room-update events.
export async function forceJoinRoom(roomId, targetUserId) {
  const res = await fetch(
    `${SYNAPSE_BASE}/_synapse/admin/v1/join/${encodeURIComponent(roomId)}`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${ADMIN_TOKEN}`,
  ...
  if (!res.ok) {
    throw new Error(data.error || `forceJoinRoom failed: ${res.status}`);
  }
  return data; // { room_id }
}
...
export async function syncMessages(accessToken, since = null) {
  ...
  const res = await fetch(`${SYNAPSE_BASE}/_matrix/client/v3/sync?${params}`, {
...
export async function joinRoom(accessToken, roomId) {
  const res = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/join`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export async function sendMessage(
  ...
  const txnId =
    customTxnId || `m.${Date.now()}.${Math.random().toString(36).slice(2, 9)}`;
  const res = await fetch(
...
export async function getOrCreateDMRoom(accessToken, myUserId, targetUserId) {
        ...
        try {
          ...
          if (stateRes.ok) {
            ...
            if (members.length === 2) {
              resolvedRoomId = rid;
              console.log(
                '[DM] ✅ Found valid 1:1 room in state:',
                resolvedRoomId,
              );
              break;
            }
            ...
            if (membersRes.ok) {
              ...
              if (memberCount === 2) {
                resolvedRoomId = rid;
                console.log(
                  '[DM] ✅ Found valid 1:1 room in joined_members:',
                  resolvedRoomId,
                );
                break;
              }
  ...
  if (!resolvedRoomId) {
    ...
    const createRes = await fetch(
      `${SYNAPSE_BASE}/_matrix/client/v3/createRoom`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
    ...
    if (!createRes.ok)
      throw new Error(createData.error || 'Failed to create DM room');
  ...
  while (!joinedSuccessfully && attempts < 3) {
    ...
    try {
      const joinRes = await fetch(
        `${SYNAPSE_BASE}/_matrix/client/v3/rooms/${encodeURIComponent(resolvedRoomId)}/join`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${accessToken}`,
...
export async function sendReaction(accessToken, roomId, eventId, key) {
  ...
  const res = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/send/m.reaction/${txnId}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export async function pinMessage(accessToken, roomId, eventId) {
  ...
  const stRes = await fetch(
    `${SYNAPSE_BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/state/m.room.pinned_events`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    },
  );
