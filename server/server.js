...
const GOOGLE_KEY =
  process.env.GOOGLE_MAP_API_KEY || 'AIzaSyAVz9omOzS8B29CEi50AkrNPMPn3JXhbcg';
...
// Run schema migrations at boot (additive, idempotent, non-blocking)
(async () => {
  try {
    const { runMigrations } = require('./migrate');
    await runMigrations(pgPool);
  } catch (err) {
    console.warn('⚠️  migrations skipped:', err.message);
  }
})();
...
(async () => {
  try {
    ...
    try {
      ...
      console.log(
        `✅ NATS connected → ${natsUrl}${tlsEnabled ? ' (TLS)' : ' (Plain)'}`,
      );
    } catch (tlsErr) {
      // If TLS failed, attempt plain text fallback
      if (tlsEnabled) {
        console.warn(
          '⚠️  NATS TLS connection failed. Attempting plain connection...',
        );
    ...
    (async () => {
      for await (const msg of sub) {
        try {
          ...
          console.log(
            `📥 NATS RECV [gps.stream.req] -> Unit: ${unitId}, Ticket: ${ticketNo}`,
          );
          ...
          const point = {
            ...
            tripStatus:
              prevState.tripStatus === 'completed' ? 'completed' : tripStatus,
          ...
          ambulanceLocation = {
            ...point,
            trail: [...ambulanceLocation.trail.slice(-149), point],
          };
    ...
    (async () => {
      for await (const msg of systemSub) {
        try {
          const data = JSON.parse(sc.decode(msg.data));
          if (
            data.alert_type === 'dispatch' ||
            data.alert_type === 'assignment'
          ) {
            const { unit_id, ticket_data } = data;
            const targetUnit = unit_id || ticket_data?.unitId || 'matrixuser';

            console.log(
              `📡 [NATS DISPATCH] Received from Webhook for Unit: ${targetUnit}`,
            );

            // 1. Sync internal state (Memory)
            if (!units.has(targetUnit)) {
              units.set(targetUnit, {
                id: targetUnit,
                name: targetUnit,
                status: 'online',
                type: ticket_data?.vehicleType || 'ambulance',
                lastSeen: Date.now(),
              });
            }
            ...
            unitTripState.set(
              key,
              makeFreshTripState({ tripStatus: 'dispatched' }),
            );
            ...
            console.log(
              `✅ [NATS DISPATCH] Successfully handled and routed to ${inboxSubject}`,
            );
          }
        } catch (err) {
      ...
      for await (const msg of querySub) {
        try {
          ...
          if (msg.subject.endsWith('.getUnits')) {
            ...
            console.log(
              `🙋 [NATS QUERY] Providing System Status to ${replyTo}`,
            );
            nc.publish(
              replyTo,
              sc.encode(JSON.stringify({ alert: currentAlert })),
            );
          } else if (msg.subject.endsWith('.getNearestUnits')) {
            console.log(
              `🙋 [NATS QUERY] Providing Nearest Units to ${replyTo}`,
            );
            ...
            const unitsArray = Array.from(units.values())
              ...
              .map((u) => ({
                ...u,
                distance: Math.sqrt(
                  Math.pow((u.location?.latitude || 0) - lat, 2) +
                    Math.pow((u.location?.longitude || 0) - lng, 2),
                ),
              }))
            ...
            nc.publish(
              replyTo,
              sc.encode(JSON.stringify({ success: true, data: unitsArray })),
            );
          } else if (data.alert_type === 'registration') {
            handleRegister(
              { body: data.unit_data },
              { json: (r) => console.log('✅ NATS Registration Done') },
            );
...
async function trackInsert({
  ...
  try {
    ...
    console.log(
      `📍 DB ✅ ${unitId} | ${tripStatus} | ${device_status} | spd=${speed}`,
    );
  } catch (err) {
...
async function broadcastPush(alert) {
  ...
  try {
    ...
    (await r.json()).data?.forEach((item, i) => {
...
app.get('/forms/:unitType', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT f.*, ft.name AS type_name FROM forms f JOIN form_types ft ON f.form_type_id = ft.id WHERE f.unit_type = ? AND f.form_type_id = 1 AND f.is_active = 1 LIMIT 1`,
      [req.params.unitType],
    );
    if (!rows.length)
...
app.get('/forms/scene/assessment', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT f.*, ft.name AS type_name FROM forms f JOIN form_types ft ON f.form_type_id = ft.id WHERE f.form_type_id = 2 AND f.is_active = 1 LIMIT 1`,
    );
    if (!rows.length)
      return res
        .status(404)
        .json({ success: false, error: 'Scene form not found' });
    res.json({ success: true, data: rows[0] });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
app.post('/forms/:formId/submit', async (req, res) => {
  try {
    ...
    const form = rows[0],
      ...
      errors = validateAnswers(fields, answers);
...
app.get('/forms/submissions/:incidentId', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT fs.*, f.name AS form_name, f.unit_type, ft.name AS form_type_name FROM form_submissions fs JOIN forms f ON fs.form_id = f.id JOIN form_types ft ON f.form_type_id = ft.id WHERE fs.incident_id = ? ORDER BY fs.submitted_at DESC`,
...
app.get('/admin/forms', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT f.id, f.name, f.unit_type, f.is_active, ft.name AS type_name, JSON_LENGTH(f.fields) AS field_count, f.created_at, f.updated_at FROM forms f JOIN form_types ft ON f.form_type_id = ft.id ORDER BY f.form_type_id, f.unit_type`,
    );
    res.json({ success: true, data: rows });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
app.post('/admin/forms', async (req, res) => {
  try {
    ...
    if (!name || !unit_type || !fields?.length)
      return res
        .status(400)
        .json({ success: false, error: 'name, unit_type, fields required' });
    const [result] = await db.query(
...
function handleRegister(req, res) {
  ...
  if (!unitTripState.has(unitId))
    unitTripState.set(unitId, makeFreshTripState({ tripStatus: 'idle' }));
  console.log(
    `✅ Unit registered: ${name} (${unitId}) — waiting for first heartbeat to record online event`,
  );
...
app.post('/heartbeat', (req, res) => {
  ...
  const hdg =
    parseFloat(req.body.heading) >= 0 ? parseFloat(req.body.heading) : 0;
...
app.get('/nearest', (req, res) => {
  try {
    ...
    const typeFilter =
      req.query.type && req.query.type !== 'null' ? req.query.type : null;
    ...
    const available = Array.from(units.values())
      .filter((u) => {
        ...
        const hasLoc =
          !isNaN(locLat) && !isNaN(locLng) && locLat !== 0 && locLng !== 0;
...
function handleLocationUpdate(req, res) {
  ...
  const point = {
    ...
    tripStatus:
      prevState.tripStatus === 'completed'
        ? 'completed'
  ...
  console.log(
    `🚑 ${unitId} | speed=${point.speed} | ETA=${point.remainingTimeS}`,
  );
...
app.get('/unit-location/:unitId', (req, res) => {
  ...
  const tripState =
    unitTripState.get(key) || makeFreshTripState({ tripStatus: 'idle' });
...
app.get('/all-locations', (req, res) => {
  const now = Date.now();
  const data = Array.from(units.values())
    .filter((u) => u.location?.latitude)
    .map((u) => ({
      id: u.id,
      name: u.name,
      type: u.type,
      ...
      heading: u.location.heading || 0,
      speed: u.location.speed || 0,
...
app.post('/assign', async (req, res) => {
  ...
  if (unit.status === 'busy')
    return res
      .status(400)
      .json({ error: `${unit.name} is already on an active incident` });
  const incidentId = uuidv4();
  let submissionId = null;
  if (formId && answers && Object.keys(answers).length > 0) {
    try {
      const [rows] = await db.query('SELECT fields FROM forms WHERE id = ?', [
        formId,
      ]);
      if (rows.length) {
        const fields =
      ...
      console.warn('DB submission save failed:', dbErr.message);
    }
  }
  ...
  // Fire-and-forget — Expo Push is a backup channel; NATS publish below is the real delivery path
  if (unit.pushToken)
    sendPushToToken(unit.pushToken, alertData).catch((err) =>
      console.error('Push error:', err.message),
    );
...
app.post('/send-alert', async (req, res) => {
  ...
  resetLegacyLocation();
  const selectedUnits = dispatchToNearestUnits(
    currentAlert,
    currentAlert.vehicleType,
  );
  // Fire-and-forget — Expo Push is a backup channel, don't block the response on it
  broadcastPush(currentAlert).catch((err) =>
    console.error('Broadcast push error:', err.message),
  );
  res.json({
    success: true,
    id,
    assignedUnits: selectedUnits.map((u) => u.id),
  });
});
...
app.get('/my-alert', (req, res) => {
  ...
  res.json({
    alert: assignments.get(unitId) || { id: null, status: 'waiting' },
  });
});
...
app.post('/accept-assignment', (req, res) => {
  ...
  for (const [id, a] of assignments.entries()) {
    if (a.id === assign.id && id !== unitId) {
      ...
      console.log(
        `🚫 Removed assignment from unit ${id} — accepted by ${unitId}`,
      );
    }
  }
...
app.post('/accept', (req, res) => {
  if (!currentAlert.id)
    return res.status(400).json({ error: 'No active alert' });
  currentAlert.status = 'accepted';
  ambulanceLocation.trail = [];
  res.json({ success: true });
});
...
app.post('/reject-assignment', (req, res) => {
  ...
  currentAlert.reason = reason;
  res.json({ success: true });
});

app.post('/reject', (req, res) => {
  currentAlert.status = 'rejected';
  currentAlert.reason = req.body.reason || 'manual';
  res.json({ success: true });
});
...
app.post('/update-dispatch-status', async (req, res) => {
  ...
  if (
    ...
    assignedUnits.every(
      (id) => unitTripState.get(id)?.tripStatus === 'completed',
    )
  ) {
...
app.get('/incidents', (req, res) =>
  res.json({
    success: true,
    data: Array.from(incidents.values()).sort(
...
app.get('/directions', async (req, res) => {
  ...
  const tMode = mode || 'driving',
    traffic =
      tMode === 'driving' ? '&departure_time=now&traffic_model=best_guess' : '';
  try {
    const data = await fetch(
      `https://maps.googleapis.com/maps/api/directions/json?origin=${originLat},${originLng}&destination=${destLat},${destLng}&mode=${tMode}&alternatives=true${traffic}&key=${GOOGLE_KEY}`,
    ).then((r) => r.json());
    res.json(data);
  } catch {
...
try {
  ...
  console.warn('⚠️  ticket-events module not loaded:', e.message);
}
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(
    `🟢 Registration online event                   → POST /register-unit`,
  );
  console.log(
    `📜 Trip history                                → GET  /track-history/:unitId`,
  );
  console.log(
    `📜 History by ticket                           → GET  /track-history/ticket/:ticketNo`,
  );
  console.log(
    `🔋 Unit status history (online/offline events) → GET  /unit-status-history/:unitId`,
  );
  console.log(
    `🔋 All units status history                    → GET  /all-status-history`,
  );
});
